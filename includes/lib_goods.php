...
function get_comment_ranks($goods_id)
{
 if (empty($goods_id))
    {
        return '5';
...
function get_categories_tree($cat_id = 0)
{
    ...
    */
    ...
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name,cat_enname,parent_id,is_show ' .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
			   $cat_arr[$row['cat_id']]['cat_enname']   = $row['cat_enname'];
...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name,cat_enname, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
        ...
        {
            if ($row['is_show'])
$three_arr[$row['cat_id']]['cat_enname'] = $row['cat_enname'];
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
			...
			$goods[$idx]['goods_crank']         = get_comment_ranks($row['goods_id']);
...
function get_goods_properties($goods_id)
{
    ...
    /* 获得商品的规格 */
    $sql = "SELECT a.attr_id, a.attr_name,g.color,g.attr_org,g.attr_thumb,g.attr_thumps,g.attr_left_thumb,g.attr_left_thumps,g.attr_left_org,g.frame_des,g.glass_des,g.frame_endes,g.glass_endes,a.attr_group,g.xinghao, a.is_linked, a.attr_type, ".
                "g.goods_attr_id, g.attr_value,g.attr_envalue, g.attr_price " .
														 ...
														 'color'        => $row['color'],
														  ...
														  'attr_thumb'=>$row['attr_thumb'],
														    'attr_thumps'=>$row['attr_thumps'],
															  ...
															  'frame_des'=>$row['frame_des'],
														    'glass_des'=>$row['glass_des'],
																  'frame_endes'=>$row['frame_endes'],
														    'glass_endes'=>$row['glass_endes'],
															'xinghao'=>$row['xinghao'],
															  'attr_left_thumb'        => $row['attr_left_thumb'],
														  'attr_left_thumps'=>$row['attr_left_thumps'],
														    'attr_left_org'=>$row['attr_left_org'],
															
                                                        'format_price' => price_format(abs($row['attr_price']), false),
                                                        'id'           => $row['goods_attr_id']);
														
	   $arr['spe'][$row['attr_id']]['count']=count( $arr['spe'][$row['attr_id']]['values']);
        }
...
{
    $sql = 'SELECT img_id, img_url, thumb_url,thumb_url_t, img_desc' .
