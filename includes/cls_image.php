...
class cls_image
{
    ...
    //缩略图最小高度
    public $thumbMixHeight;
    //缩略图最小宽度
    public $thumbMixWidth;
    var $error_no    = 0;
    var $error_msg   = '';
     ...
     */
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='#f6f6f6')
    {
     ...
     * @param int $Type     =1 调整尺寸 =2 缩略图剪切 
     * @param int $thumb_w 缩略图宽
     * @param int $thumb_h 缩略图高
     * @param string $img_suffix 缩略图后缀
     * @param string $t_suffix 调整尺寸图后缀
     */
    static function cutImg($image, $thumb_w, $thumb_h, $img_suffix, $Type=1,$t_suffix="")
    {
        ...
        if($Type==2){
            $img_tmp = str_replace(".", $img_suffix.".", $image);
        }else{
             $img_tmp = str_replace(".", $img_suffix.".", $image);
        }
        ...
        if ($Type == 1) {
            ...
            if ($h / $w > $thumb_h / $thumb_w) { #高比较大
                $width = $thumb_w;
                $height = $h * $thumb_w / $w;
                $IntNH = $height - $thumb_h;
                if($w<$thumb_w){
                    $thumb_w =$w;
                }
                $nImg = imagecreatetruecolor($thumb_w, $thumb_h);
                imagecopyresampled($nImg, $Img, 0, 0, 0, 0, $thumb_w, $height, $w, $h);
            } else {     #宽比较大
                $height = $thumb_h;
                $width = $w * $thumb_h / $h;
                $IntNW = $width - $thumb_w;
                ...
                $nImg = imagecreatetruecolor($thumb_w, $thumb_h);
                imagecopyresampled($nImg, $Img, -$IntNW / 1.8, 0, 0, 0, $width, $thumb_h, $w, $h);
            }
