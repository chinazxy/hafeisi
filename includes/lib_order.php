...
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name,s.name, ' .
            ...
            'WHERE  a.shipping_id = s.shipping_id AND s.enabled = 1 and s.is_show=1';

    return $GLOBALS['db']->getAll($sql);
}

function available_shipping_list_t($region_id_list)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name,s.name, ' .
                's.shipping_desc, s.insure, s.support_cod, a.configure ' .
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' .
            ...
            'WHERE  a.shipping_id = s.shipping_id AND s.enabled = 1';
...
{
    $sql = 'SELECT s.shipping_code, s.shipping_name,s.name, ' .
                's.shipping_desc, s.insure, s.support_cod, a.configure ' .
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' .
                $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' .
                $GLOBALS['ecs']->table('area_region') . ' AS r ' .
...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
	...
	if($mobile==0){
	    $sql = 'SELECT pay_id, pay_code,pay_icon,pay_mp_ico,pay_mp_back_ico,pay_name, pay_fee, pay_desc, pay_config, is_cod' .
            ' FROM ' . $GLOBALS['ecs']->table('payment') .
            ' WHERE enabled = 1 and is_pc=1 ';	
		
	}else{
	    $sql = 'SELECT pay_id, pay_code,pay_icon,pay_mp_ico,pay_mp_back_ico,pay_name, pay_fee, pay_desc, pay_config, is_cod' .
            ' FROM ' . $GLOBALS['ecs']->table('payment') .
            ' WHERE enabled = 1 and is_mp=1  ';	
...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            "goods_price, goods_attr,goods_attr_id, is_real, parent_id, is_gift, " .
            "goods_price * goods_number AS subtotal, extension_code,dingzhi_attr,dingzhi_attr_list,dingzhi_thumb,dingzhi_img,dingzhi_number,dingzhi_side ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if ($row['extension_code'] == 'package_buy')
        {
			...
			for($pa=0;$pa<count($row['package_goods_list']);$pa++){
			
		    $goodsprice   =  get_user_rank_prices($row['package_goods_list'][$pa]['goods_id'],$row['package_goods_list'][$pa]['shop_price']);
			$row['package_goods_list'][$pa]['price']= $goodsprice['price'];
			
			$packageprice+=$goodsprice['price'];
			
			}
		
			  $row['shop_price']=$packageprice;
	
        }else{
		
		  $sql="SELECT g.shop_price,g.goods_thumb  FROM ".$GLOBALS['ecs']->table('goods')." as g WHERE goods_id=".$row['goods_id'];
		  
		  $goodsinfo=$GLOBALS['db']->getRow($sql);
		  
		  $row['shop_price']=$goodsinfo['shop_price'];
		  
		   $row['goods_thumb']=$goodsinfo['goods_thumb'];

		  
		  
		  	   	    if ($row['goods_attr_id']>0){
		
		 $attrid=explode(",",$row['goods_attr_id']);
		  $attrpice=0;
		 ...
		 $row['shop_price']=$row['shop_price']+$attrpice;
		  ...
		  $rssf=array();
		   ...
		   if($attrid[$i]>0){
		   $sql = "SELECT attr_value,color,attr_id,frame_des,glass_des FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_attr_id= " .$attrid[$i];
		  
			 $rssf[$i]=$GLOBALS['db']->getRow($sql);
			 if($rssf[$i]['attr_id']>0){
			$sqls = "SELECT attr_name FROM " . $GLOBALS['ecs']->table('attribute') . " WHERE attr_id= " .$rssf[$i]['attr_id'];
			$aname= $GLOBALS['db']->getRow($sqls);
			$rssf[$i]['name']=$aname['attr_name'];
			}
			}
			}
...
function order_fee($order, $goods, $consignee)
{
    ...
    foreach ($goods AS $val)
    {
        ...
        if($val['extension_code']=="dingzhi"){

               $total['market_price'] += $val['goods_price'] * $val['goods_number'];
        }else{
        $total['market_price'] += $val['shop_price'] * $val['goods_number'];
       }
    }
    ...
    $total['jiesheng']= $total['market_price']-$total['goods_price']+$total['discount']+$total['bonus'];
	 $total['formart_jiesheng']= price_format($total['jiesheng'],false);
...
{
    $res=$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'),$order, 'UPDATE', "order_id = '$order_id'");
				...
				if(count($result)>1 && $dizcount!=count($result) && $dizcount!=0){
			        ...
			        " SET order_amount = '$goods_amount' WHERE log_id = '$log_id'";
					 ...
					 for($i=0;$i<$dizcount;$i++){
					  ...
					  "select  '".$new_order_sn[$i]."'  as order_sn,'".$new_order_d_sn[$i]."' as order_d_sn,user_id, order_status, shipping_status, pay_status, consignee, surname, ming, quhao, haoma, room, country,".
...
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number,dingzhi_attr,dingzhi_img,dingzhi_thumb,dingzhi_attr_list, " .
            "market_price, goods_price, goods_attr,goods_attr_id, is_real, extension_code, parent_id, is_gift, is_shipping, " .
    ...
    {
	       $sql="SELECT g.goods_id,g.shop_price,g.goods_thumb from ".$GLOBALS['ecs']->table('goods') ." as g WHERE goods_id=".$value['goods_id'];
		$goodsinfo=$GLOBALS['db']->getRow($sql);
		...
		$arr[$key]['goods_thumb']     = $goodsinfo['goods_thumb'];
		
			
        $arr[$key]['brand_name']     = $goodsinfo['brand_name'];
        if ($value['extension_code'] == 'package_buy')
        {
		    $packageprice=0;
            $arr[$key]['package_goods_list'] = get_package_goods($value['goods_id']);
			
			for($pa=0;$pa<count($arr[$key]['package_goods_list']);$pa++){
			
		    $goodsprice   =  get_user_rank_prices($arr[$key]['package_goods_list'][$pa]['goods_id'],$arr[$key]['package_goods_list'][$pa]['shop_price']);
			$arr[$key]['package_goods_list'][$pa]['price']= $goodsprice['price'];
	    ...
	    if($packageprice>0){
       $arr[$key]['discount']=round(($value['goods_price']/$packageprice),3)*10;
		}else{
		$arr[$key]['discount']=10;
		}
		...
		$arr[$key]['formart_shop_price']=price_format($packageprice,false);
		$arr[$key]['shop_price']=$packageprice;
		
	
		
        }else if($value['extension_code'] == 'dingzhi'){
		  $arr[$key]['dingzhi_url']="dingzhi.php?url=".encrypt_url("act=dingzhi_edit&id=".$value['rec_id']."&time=".time(),URL_KEY)." ";
			
			$arr[$key]['dingzhi_attr_list_str']=unserialize($value['dingzhi_attr_list']);
			if($arr[$key]['dingzhi_attr_list_str']['maxtime']>$GLOBALS['_CFG']['dingzhi_day']){
			$arr[$key]['shipping_date']=date("Y-m-d",strtotime('+'.$arr[$key]['dingzhi_attr_list_str']['maxtime'].' day'));
			$arr[$key]['accessories']=$arr[$key]['dingzhi_attr_list_str']['maxtime_title'];
			 }else{
			 	$arr[$key]['shipping_date']=date("Y-m-d",strtotime('+'.$GLOBALS['_CFG']['dingzhi_day'].' day'));
				$arr[$key]['accessories']="";
			 }
		
		$arr[$key]['shop_price']=$row['goods_price'];
		}else{
		
		
				    if (!empty($value['goods_attr_id'])){
		 $attrid=explode(",",$value['goods_attr_id']);
		  $attrpice=0;
	
		   for($t=0;$t<count($attrid);$t++){
			 $attrpice+=$GLOBALS['db']->getOne("SELECT attr_price FROM ". $GLOBALS['ecs']->table('goods_attr') ." WHERE goods_attr_id=".$attrid[$t]);
			}
		// $attr_price=$GLOBALS['db']->getOne("SELECT attr_price FROM ". $GLOBALS['ecs']->table('goods_attr') ."WHERE goods_attr_id=$value[goods_attr_id]");
		// echo $goodsinfo['shop_price'];
		 $shopprice=$goodsinfo['shop_price']+$attrpice;
		$arr[$key]['formart_shop_price']= price_format($shopprice,true);
		$arr[$key]['shop_price']=$shopprice;
		}else{
		$arr[$key]['formart_shop_price']=price_format($goodsinfo['shop_price'],false);
		$arr[$key]['shop_price']=$goodsinfo['shop_price'];
		
		}
			if($goodsinfo['shop_price']>0){
       $arr[$key]['discount']=round(($value['goods_price']/$arr[$key]['shop_price']),2)*10;
		}else{
		$arr[$key]['discount']=10;
		}
		
		 $arr[$key]['discountss']=$value['goods_price'];
		
		 $arr[$key]['discountssff']=$goodsinfo['shop_price'];
				$shopprice=0;
		  ...
		  $rssf=array();
		   for($i=0;$i<count($attrid);$i++){
		   $sql = "SELECT attr_value,attr_id,attr_thumb,attr_left_thumb,frame_des,glass_des FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_attr_id= " .$attrid[$i];
		 
			 $rssf[$i]=$GLOBALS['db']->getRow($sql);
			 $arr[$key]['frame_des']=mb_substr($rssf[$i]['frame_des'],0,2,"utf-8");
			 $arr[$key]['glass_des']=mb_substr($rssf[$i]['glass_des'],0,2,"utf-8");
			$sqls = "SELECT attr_name FROM " . $GLOBALS['ecs']->table('attribute') . " WHERE attr_id= " .$rssf[$i]['attr_id'];
			$aname= $GLOBALS['db']->getRow($sqls);
			$rssf[$i]['name']=$aname['attr_name'];
			}
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
	...
	$specstr=implode(",",$spec);
	    ...
	    $goodsinfo= $GLOBALS['db']->getRow("SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart'). " WHERE goods_id=".$goods_id."  and goods_attr_id='".$specstr."' and session_id = '" . SESS_ID . "' ");
	
        $numers=$goodsinfo['goods_number'];
	  $goodsnum=0;
	   $goodsnum= $num+(int)$numers;

    /* 检查：库存 */
    if ($GLOBALS['_CFG']['use_storage'] == 1)
    {
        //检查：商品购买数量是否大于总库存
        if ($goodsnum > $goods['goods_number'])
        {
        ...
        {  
            ...
            {
                /* 取规格的货品库存 */
                if ($goodsnum > $product_info['product_number'])
                {
...
function get_cart_goods()
{
    ...
    {    
		      ...
		      if ($row['extension_code'] == 'package_buy')
        {
			...
			for($pa=0;$pa<count($row['package_goods_list']);$pa++){
			   ...
			   $goodsprice   =  get_user_rank_prices($row['package_goods_list'][$pa]['goods_id'],$row['package_goods_list'][$pa]['shop_price']);
			    ...
			    $packageprice+=$goodsprice['price'];
			  $packageTotal+=$row['package_goods_list'][$pa]['price'];
			   
			} 
			
			$totalprice+=$packageTotal*$row['goods_number'];
		
			if($packageTotal>0){
			

        $row['discount']=round(($row['goods_price']/$packageTotal),3)*10;
		}else{
		$row['discount']=10;
		}
		  $row['shop_price']=  $packageTotal;
		   ...
		   $row['formart_shop_price']= price_format($packageTotal,true);
			...
			$row['dingzhi_attr_list_str']=unserialize($row['dingzhi_attr_list']);
			$totalprice=$totalprice+$row['goods_price']* $row['goods_number'];
		}else{
		
	    $row['shop_price']=$GLOBALS['db']->getOne("SELECT shop_price FROM ". $GLOBALS['ecs']->table('goods') ."WHERE goods_id=$row[goods_id]");
		$row['cat_id']=$GLOBALS['db']->getOne("SELECT cat_id FROM ". $GLOBALS['ecs']->table('goods') ."WHERE goods_id=$row[goods_id]");
		$row['cat_name']=$GLOBALS['db']->getOne("SELECT cat_name FROM ". $GLOBALS['ecs']->table('category') ."WHERE cat_id=$row[cat_id]");
		...
		$totalprice=$totalprice+$row['shop_price']* $row['goods_number'];
		
		    	
		if($row['shop_price']>0){
        $row['discount']=round(($row['goods_price']/$row['shop_price']),2)*10;
		}else{
		$row['discount']=10;
		}
      ...
      //  $total['market_price'] += $row['market_price'] * $row['goods_number'];
        ...
        else
        {
		  ...
		  $rssf=array();
		   for($i=0;$i<count($attrid);$i++){
		   $sql = "SELECT attr_value,color,attr_id,attr_thumb,attr_left_thumb,frame_des,glass_des FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_attr_id= " .$attrid[$i];
		
			 $rssf[$i]=$GLOBALS['db']->getRow($sql);
			 $row['frame_des']=mb_substr($rssf[$i]['frame_des'],0,2,"utf-8");
			 $row['glass_des']=mb_substr($rssf[$i]['glass_des'],0,2,"utf-8");
			 if(!empty($rssf[$i]['attr_thumb'])){
			 
			 $row['attr_thumb']=$rssf[$i]['attr_thumb'];
			 }
			  if(!empty($rssf[$i]['attr_left_thumb'])){
			 
			 $row['attr_left_thumb']=$rssf[$i]['attr_left_thumb'];
			 }
			$sqls = "SELECT attr_name FROM " . $GLOBALS['ecs']->table('attribute') . " WHERE attr_id= " .$rssf[$i]['attr_id'];
			$aname= $GLOBALS['db']->getRow($sqls);
			$rssf[$i]['name']=$aname['attr_name'];
...
$row['goodsattr']=$rssf;
        ...
        if (trim($row['goods_attr']) != '')
        {
           ...
           $goodsinfo = $GLOBALS['db']->getRow("SELECT `goods_thumb`,brand_id FROM " . $GLOBALS['ecs']->table('goods') . " WHERE `goods_id`='{$row['goods_id']}'");
			...
			$row['brand_id'] =$goodsinfo['brand_id'];
