...
{
    $sql = 'SELECT g.goods_id, g.goods_name,c.attr_id as attr_val,at.attr_name,a.attr_value,a.attr_thumb as attr_thumbs,a.attr_id,a.attr_value as colors,a.goods_attr_id, g.goods_sn,g.goods_number, g.market_price,g.goods_thumb,g.brand_id, g.shop_price AS org_price, '.
			...
			" LEFT JOIN ".$GLOBALS['ecs']->table('goods_attr')." AS a ON c.goods_id=a.goods_id AND c.attr_id=a.goods_attr_id  LEFT JOIN ". $GLOBALS['ecs']->table('attribute').
            " as at on at.attr_id=a.attr_id LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ".
...
//$goods_list[$row['goods_id']]['brand_name']=$GLOBALS['db']->getOne("SELECT brand_name FROM " .$GLOBALS['ecs']->table('brand')." where brand_id=".$row['brand_id']);
        
		$time = time();
		$time1= strtotime(date("y-m-d",$time));
		$goods_list[$key]['rec_id']        = $row['rec_id'];
		$time2=$goods_list[$key]['add_time']        = $row['add_time'];
		$time3= strtotime(date("y-m-d",$time2));
		$time4 = ($time1 - $time3) / 86400;
		$goods_list[$key]['add_time']        = $time4;
        $goods_list[$key]['is_attention']  = $row['is_attention'];
        $goods_list[$key]['goods_id']      = $row['goods_id'];
		 $goods_list[$key]['attr_name']      = $row['attr_name'];
		  $goods_list[$key]['attr_id']      = $row['attr_val'];
        $goods_list[$key]['goods_name']    = $row['goods_name'];
		$goods_list[$key]['goods_sn']    = $row['goods_sn'];
		$goods_list[$key]['color']    = $row['colors'];
		$goods_list[$key]['goods_thumb']    = $row['attr_thumbs'];
		$goods_list[$key]['goods_number']    = $row['goods_number'];
		 
		$goods_list[$key]['page']    = ceil($key/6);
        $goods_list[$key]['market_price']  = price_format($row['market_price']);
        $goods_list[$key]['shop_price']    = price_format($row['shop_price']);
		 $goods_list[$key]['s_shop_price']    = $row['shop_price'];
