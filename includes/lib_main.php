...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_money,u.name,u.surname,u.email, u.pay_points, u.user_rank, u.rank_points, '.
...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    $ur_here    = '<li><a href="index.php">' . $GLOBALS['_LANG']['home'] . ' &gt;</a> </li> <li><a href="category.php">' . $GLOBALS['_LANG']['l_product_c'].' &gt;</a></li>';
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .= ' <li> <a href="' . build_uri($type, $args, $val['cat_name']) . '#'.$cat.'">' .
    ...
    if (!empty($str))
    {
        ...
        $ur_here    .= ' <li><span style="color: #000"> ' . $str . '</span></li>';
    }
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='',$month,$deparment)
{
    ...
    switch ($app)
    {
	   ...
	   case 'product':
            $uri_args = array('cid' => $cat,'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
            break;
				   case 'men':
            $uri_args = array('cid' => $cat,'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
            break;
        ...
        case 'article_cat':
            ...
            break;
			   case 'brand':
            $uri_args = array('acid' => $cat, 'sort' => $sort, 'order' => $order,'month'=>$month);
            break;
		     case 'product':
            $uri_args = array('acid' => $cat, 'sort' => $sort, 'order' => $order);
            break;
           case 'brand':
            ...
            break;
			   ...
			   case 'joinlist':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'sort' => $sort, 'order' => $order, 'display' => $display_type,'deparment'=>$deparment);
            break;
        case 'search':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'sort' => $sort,'filter_attr'=>$filter_attr, 'order' => $order);
            break;
        case 'exchange':
...
function get_dyna_libs($theme, $tmp)
{
    ...
    $ext = end(array_shift(explode('.', $tmp)));
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
  ...
  $gca=getCatArticle();
		  $GLOBALS['smarty']->assign('help_list', $gca['help_list']);
     $GLOBALS['smarty']->assign('article', $gca['artlist']); 
...
function show_register_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
    assign_template();
  ...
  $gca=getCatArticle();
    ...
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('register_message.dwt');
