...
function edit_profile($profile)
{
    ...
    /* 过滤非法的键值 */
    $other_key_array = array('name','surname','msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone');
...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT user_name,name, birthday, sex, question,surname, answer, rank_points, pay_points,user_money, user_rank,".
    ...
    $info['sex']         = isset($infos['sex'])      ? $infos['sex']      : 1;
...
function get_consignee_all_list($user_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
 ...
 $row['fptime']=date("Y年n月d日",$row['add_time']);
 
 			 $row['inv_content_name']=$inv_content_list[$row['inv_content']];
	   $row['inv_type_name']=$inv_type_xl[$row['inv_type']];
...
$goodsids[$i]['extension_code']=$goodsinfo[$i]['extension_code'];


		if($goodsinfo[$i]['extension_code']=="package_buy"){
		
		   $sql="SELECT goods_id  FROM ".$GLOBALS['ecs']->table('package_goods') ."  where package_id=".$goodsinfo[$i]['goods_id'];
		   $idarrays=$GLOBALS['db']->getAll($sql);
		   for($t=0;$t<count($idarrays);$t++){
		    $sql="SELECT a.goods_name,a.goods_thumb,a.shop_price FROM ".$GLOBALS['ecs']->table('goods') ." as a  where a.goods_id=".$idarrays[$t]['goods_id'];
			
		     $goodsids[$i]['package_buy'][$t]= $GLOBALS['db']->getRow($sql);
			 $goodsids[$i]['package_buy'][$t]['number']=$goodsinfo[$i]['goods_number'];
		   }

		}else{
            $sql="SELECT a.goods_name,a.goods_sn,a.goods_id,a.goods_thumb,a.shop_price FROM ".$GLOBALS['ecs']->table('goods') ." as a  where a.goods_id=".$goodsinfo[$i]['goods_id'];
			
		    $goodsids[$i]= $GLOBALS['db']->getRow($sql);
			
			
		   if(!empty($goodsinfo[$i]['goods_attr']) && $goodsinfo[$i]['goods_attr_id']>0){
		
		    $attr=explode(":",$goodsinfo[$i]['goods_attr']);
			$attrs=explode("[",$attr[1]);
			$attr=$attrs[0];
			
			//$attr_price=$GLOBALS['db']->getOne("SELECT attr_price FROM ". $GLOBALS['ecs']->table('goods_attr') ."WHERE goods_attr_id=".$goodsinfo[$i]['goods_attr_id']);
			
					  	   	    if ($goodsinfo[$i]['goods_attr_id']>0){
		
		 $attrid=explode(",",$goodsinfo[$i]['goods_attr_id']);
		  
	 $attr_price=0;
		   for($t=0;$t<count($attrid);$t++){
			 $attr_price+=$GLOBALS['db']->getOne("SELECT attr_price FROM ". $GLOBALS['ecs']->table('goods_attr') ."WHERE goods_attr_id=".$attrid[$t]);
			}
		     ...
		     $goodsids[$i]['shop_price']= price_format($goodsids[$i]['shop_price']+$attr_price, false);
		 
		   $goodsids[$i]['attr']=$attr;
		   $goodsids[$i]['rec_id']=$goodsinfo[$i]['rec_id'];
		   $goodsids[$i]['number']=$goodsinfo[$i]['goods_number'];
		   $goodsids[$i]['refund_status']=$goodsinfo[$i]['refund_status'];
$goodsids[$i]['refund_confirm_desc']=$goodsinfo[$i]['refund_confirm_desc'];
            if($goodsinfo[$i]['extension_code']=='dingzhi'){
           $goodsids[$i]['dingzhi_attr']=$goodsinfo[$i]['dingzhi_attr'];
           $goodsids[$i]['dingzhi_thumb']=$goodsinfo[$i]['dingzhi_thumb'];
           $goodsids[$i]['dingzhi_attr_list']=$goodsinfo[$i]['dingzhi_attr_list'];
           $goodsids[$i]['extension_code']=$goodsinfo[$i]['extension_code'];
           $goodsids[$i]['goods_name']=$goodsinfo[$i]['goods_name'];
             $goodsids[$i]['goods_sn']=$goodsinfo[$i]['goods_sn'];
             $goodsids[$i]['goods_total']=price_format($goodsinfo[$i]['goods_number']*$goodsinfo[$i]['goods_price']);
           }
		   		  if ($goodsinfo[$i]['goods_attr_id'] != ''){
		  
		   $attrid=explode(",",$goodsinfo[$i]['goods_attr_id']);
		   $rssf=array();
		 //  print_r($attrid);
		   for($g=0;$g<count($attrid);$g++){
		   $sql = "SELECT attr_value,color,attr_id,attr_thumb FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_attr_id= " .$attrid[$g];
		
			 $rssf[$g]=$GLOBALS['db']->getRow($sql);
			 
			
			 if(!empty($rssf[$g]['attr_id'])){
			$sqls = "SELECT attr_name FROM " . $GLOBALS['ecs']->table('attribute') . " WHERE attr_id= " .$rssf[$g]['attr_id'];
		//	echo $sqls;
			$aname= $GLOBALS['db']->getRow($sqls);
			}
			$rssf[$g]['name']=$aname['attr_name'];
				
			}
		  
		  }
		  
		  $goodsids[$i]['goodsattr']=$rssf;
		}
