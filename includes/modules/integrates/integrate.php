...
class integrate
{
    ...
    function login($username, $password, $remember = null)
    {
	    ...
	    function qqlogin($username, $password, $remember = null)
    {
	
        if ($this->check_sq_user($username, $password) > 0)
        {
            if ($this->need_sync)
            {
                $this->sync($username,$password);
    ...
    function check_user($username, $password = null)
    {
	    ...
	    function check_sq_user($username, $password = null)
    {
    ...
    function set_cookie($username='', $remember= null )
    {
        ...
        elseif ($remember)
        {
            ...
            if ($row)
            { 
		        setcookie("ECS[email]", $row['email'], $time, $this->cookie_path, $this->cookie_domain);
    ...
    function set_session ($username='')
    {
        ...
        {
            $sql = "SELECT user_id, password, email,name FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
