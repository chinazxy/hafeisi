...
// $total_fee = floatval($order['order_amount']) * 100;
 $total_fee=1;
  ...
  //var_dump($order);
  //exit;
  $name=$order['order_sn'].$order['order_id'];
 // echo $return_url;
 //exit;
// $callback_url="http://".$_SERVER['HTTP_HOST']."/xinbolon/includes/modules/payment/waptenpay/payReturnUrl.php";
 //$return_url="http://".$_SERVER['HTTP_HOST']."/xinbolon/includes/modules/payment/waptenpay/payNotifyUrl.php";
...
//$payment['pay_button']
        $button = '<div style="text-align:center"><input type="button" onclick="window.open(\''.$reqUrl.'\')" value="'."财付通wap支付".'" class="tenpay pay_btn" /></div>';
