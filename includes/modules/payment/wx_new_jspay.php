...
$payment_lang = ROOT_PATH . 'language/' .$GLOBALS['_CFG']['lang']. '/payment/wx_new_jspay.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    $modules[$i]['config']  = array(
        array('name' => 'appid',           'type' => 'text',   'value' => 'wx51c2643c7d3ee000'),
        array('name' => 'mchid',               'type' => 'text',   'value' => ''),
        array('name' => 'key',           'type' => 'text',   'value' => 'olGFy9RpW6F3dMvgd1JAcBCrJRMk22hQuoEGestWiPm'),
        array('name' => 'appsecret',           'type' => 'text',   'value' => '69a314663c627a3ca30d9342f562b2d1 '),
...
class wx_new_jspay
{
    function __construct()
    {
        ...
        if(!defined('WXAPPID'))
        {
            ...
            define('WXSSLCERT_PATH',dirname(__FILE__).'/wxpay/cacert/apiclient_cert.pem');
            define('WXSSLKEY_PATH',dirname(__FILE__).'/wxpay/cacert/apiclient_key.pem');
        }
        require_once(dirname(__FILE__)."/wxpay/WxPayPubHelper.php");
    ...
    function get_code($order, $payment)
    {
        ...
        if($openid)
        {
            ...
            $unifiedOrder->setParameter("notify_url",$GLOBALS['ecs']->url().'wx_native_callback.php');//通知地址 
            ...
            $html .= '</script>';
            $html .= '<button  class="pay_bottom c-btn4"  type="button" onclick="callpay()">微信支付</button>';
