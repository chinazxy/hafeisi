...
function getInfo($id)
{
    $url = 'http://api.hiro.net.cn/hanfeishi/info';
$pdata = array("id" => $id);
    $ch = curl_init();
...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name,c.cat_enname,c.cat_addtype, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
...
function cat_dingzhi_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;
    ...
    if ($res === NULL)
    {
            ...
            $sql = "SELECT c.cat_id, c.cat_name,c.cat_enname,  c.parent_id, c.resort, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['ecs']->table('dingzhi_jp') . " AS c ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('dingzhi_jp') . " AS s ON s.parent_id=c.cat_id ".
                "GROUP BY c.cat_id ".
                'ORDER BY c.parent_id, c.resort ASC';
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = build_uri('dingzhi_jp', array('cid' => $value['cat_id']), $value['cat_name']);
        }
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.link_thump_logo, b.brand_desc FROM " . $GLOBALS['ecs']->table('brand') . " AS b WHERE b.is_show = 1  order by b.sort_order ASC";
...
function price_no_format($price, $change_price = true)
{
    if ($change_price && defined('ECS_ADMIN') === false)
    {
        switch ($GLOBALS['_CFG']['price_format'])
 ...
 * @return  void
 */
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0,$month=0,$deparment='')
{
    static $rewrite = NULL;
        ...
        case 'brand':
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (!empty($keywords))
                    {
                        $uri .= '&amp;position=' . $keywords;
                    }
					   if (!empty($deparment))
                    {
                        $uri .= '&amp;deparment=' . $deparment;
                    }
        ...
        case 'snatch':
            ...
            else
            {
                ...
                else
                {
                    ...
                    //$uri = 'search.php?keywords=' . $keywords;
...
{
    $sql = "SELECT g.shop_price,g.brand_id,pg.goods_id,g.goods_thumb, g.goods_name, pg.goods_number, p.goods_attr,p.product_number, p.product_id
            FROM " . $GLOBALS['ecs']->table('package_goods') . " AS pg
    ...
    { 
		...
		$sql="SELECT brand_name FROM  ".$GLOBALS['ecs']->table('brand') . " WHERE brand_id=".$_row['brand_id'];
		
		$_row['brand_name']=$GLOBALS['db']->getOne($sql);
		
		$_row['shop_price']= $_row['shop_price'];
		
		$_row['farmart_shop_price']= price_format($_row['shop_price'], false);
    ...
    foreach ($row as $key => $value)
    {
        if ($value['goods_attr'] != '')
        {
            ...
            foreach ($goods_attr_array as $_attr)
            {
               ...
               $row[$key]['goods_name'] = $value['goods_name'];
			   $row[$key]['goods_number'] = $value['goods_number'];
