...
{
    $sql = 'SELECT pay_id, pay_code,pay_icon , pay_name, pay_fee, pay_desc, pay_config, is_cod' .
            ' FROM ' . $GLOBALS['ecs']->table('payment') .
            ' WHERE enabled = 1 ';
    if (!$support_cod)
    {
...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            ...
            "goods_price * goods_number AS subtotal, extension_code,dingzhi_attr,dingzhi_attr_list,dingzhi_thumb,dingzhi_img ".
        ...
        if ($row['extension_code'] == 'package_buy')
        {
		 ...
		 $row['shop_price']=$row['shop_price']+$attrpice;
		  ...
		  $rssf=array();
		   ...
		   if($attrid[$i]>0){
		   $sql = "SELECT attr_value,color,attr_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_attr_id= " .$attrid[$i];
	  ...
	  $sql = 'SELECT order_id, user_id, order_sn, consignee, address, tel, shipping_id, extension_code, extension_id, goods_amount ' .
				...
				foreach($result as $k=>$v){
					...
					if($v['extension_code']=='dingzhi'){
						...
						//$dingzhi_goods_price[$i]=$v['goods_price'];
						$dingzhi_goods_price_total=$dingzhi_goods_price_total+$v['goods_price'];
						$goods_amount=$goods_amount-$v['goods_price'];
						$i++;
					}
				}
				...
				//echo count($dingzhi_rec_id)."<br/>";
				if(count($result)>1&&count($dingzhi_rec_id)!=count($result)&&count($dingzhi_rec_id)!=0){	
				  //  echo 'din';
					$sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') .
					  ...
					  $add_time=gmtime();
					   $sql="insert into ". $GLOBALS['ecs']->table('order_info') ."(order_sn, user_id, order_status, shipping_status,".
					  ...
					  "select  '$new_order_sn', user_id, order_status, shipping_status, pay_status, consignee, surname, ming, quhao, haoma, room, country,".
					  ...
					  "inv_tel, inv_email, inv_number, inv_company_name, inv_province, inv_city, inv_district, inv_address, inv_room, inv_zipcode,'$dingzhi_goods_price_total', shipping_fee,".
					  "insure_fee, pay_fee, pack_fee, card_fee, '$dingzhi_goods_price_total', surplus, integral, integral_money, bonus, order_amount, from_ad, referer, '$add_time', confirm_time,".
				...
				for($i=0;$i<count($dingzhi_rec_id);$i++){
       ...
       $arr[$key]['discount']=round(($value['goods_price']/$arr[$key]['shop_price']),2)*10;
		...
		$arr[$key]['discount']=10;
		 ...
		 $arr[$key]['discountssff']=$goodsinfo['shop_price'];
		  ...
		  $rssf=array();
		   for($i=0;$i<count($attrid);$i++){
		   $sql = "SELECT attr_value,color,attr_id,	attr_thumb FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_attr_id= " .$attrid[$i];
