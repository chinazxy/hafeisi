...
function orderShippingSelectedResponse(result)
{
  ...
  //document.getElementById('shtype').innerHTML =result.shipping_name;
  if(result.error=="1"){
  ...
  document.getElementById('shtype_s').innerHTML =result.shipping_name;
  
  if(result.fee_type>0){
	  
	  document.getElementById('shpping_feed').innerHTML ="运费:"+result.shpping_fee; 
  }else{
	  
	 document.getElementById('shpping_feed').innerHTML ="免费";  
  }
...
function validateMbBonusResponse(obj)
{
...
{ 
  ...
  var objPayee   = document.getElementById('ECS_INVPAYEE');
  var objContent = document.getElementById('ECS_INVCONTENT');
  var needInv    = (obj.value==1) ? 1 : 0;
 
  var invType    = (obj.value==1) ? (objType != undefined ? objType.value : '') : '';
  var invPayee   = (obj.value==1) ? objPayee.value : '';
  var invContent = (obj.value==1) ? objContent.value : '';
  objType.disabled = objPayee.disabled = objContent.disabled = ! (obj.value==1);
  if(objType != null)
  {
    objType.disabled = ! obj.selected;
  }
