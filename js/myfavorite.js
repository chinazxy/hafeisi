 ...
 $(function(){
  ...
  nowpage=initallpage;
	...
	var  favoritelen= $("ul",$(".favorite").eq(nowpage)).length;

	var favoriteheight=favoritelen*205;
	
	$(".favorite").eq(nowpage).css("height",favoriteheight+"px");
...
function Page(page,obj){
 ...
 if(page>0){
    var pstep=page-1;
	   $(obj).addClass("select").siblings().removeClass("select");
    var  favoritelen= $("ul",$(".favorite").eq(pstep)).length;
	var favoriteheight=favoritelen*205;
	 $('html,body').animate({scrollTop: 200}, 500);
	$(".favorite").eq(pstep).css("height",favoriteheight+"px");
     $(".favorite").eq(pstep).fadeIn().siblings().fadeOut();
