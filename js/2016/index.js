$(function(){
  ...
  function initSlide(){
	...
	$(".worddiv").height(libh);
	$(".glt").css({top:"50%",marginTop:-($(".glt").height()+$(".gtb").height()+25)/2});
	if(w>1180){
...
function ShowLbImage(obj) {
 ...
 $(".video_ico").bind("click",function(){
 
           var objsf=$(this).parent().next().next().find("#example_video_1");
	    var videos=$("video",objsf);
	    videos.get(0).pause();
	    $(this).parent().next().next().css("width","100%");
		$(this).parent().next().next().find("video").css({height:"100%",width:"100%"});
	   $(".vjs-big-play-button",$(this).parent().next().next()).trigger("click");
	   $(this).parent().hide();
     //  objsf.play();
 });
...
var openPhotoSwipe = function() {
	...
	var items=[];
   for(var i=0; i<$(".thumbsSlideshow ul li").length;i++){
	   
	  items.push({src:$(".thumbsSlideshow ul li").eq(i).attr("data-src"),w:1500,h:2144});
 ...
 if ($(window).on("scroll",
                function() {
				   ...
				   var n = $("#explosion");
                    ...
                    for (var s = 0,
                    o = 51; o >= 0;) 
                   n.attr("data-" + (350-s) + "-bottom", "background-position:0 -" +(25100-491 * o) + "px"),
                    s % 12 === 1 && o--,
                    s++; {
