	...
	function _build($target, data) {
		if (!$target.data("zoomer")) {
			...
			if (data.controls.zoomIn || data.controls.zoomOut || data.controls.next || data.controls.previous) {
				...
				html += '<span class="zoomer-zoom-out" title="缩小" alt="缩小">-</span>';
				html += '<span class="zoomer-zoom-in" title="放大" alt="放大">+</span>';
				html += '<span class="zoomer-close" title="关闭" alt="关闭">×</span>';
				html += '</div>';
				...
				data.controls.$close = data.$zoomer.find(".zoomer-close");
			}
			...
			data.controls.$close.on("click.zoomer", data, _zoomCloseOut);
