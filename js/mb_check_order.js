$(function() {
  ...
  $(".del_sure_btn").click(function(){
	  var redid=$(this).attr("recid");
	  
	  var carobjs=$(this).parent().next().find(".caca");
      drop_goods(redid, carobjs,$(this));
 
  });
  
  $(".cadres").click(function(){
	  
	  if($(this).hasClass("miaoaoff")){
		  $(this).removeClass("miaoaoff").addClass("miaoaon");
		  $(".miao_xl").show();
		  
	  }else{
		   $(this).removeClass("miaoaon").addClass("miaoaoff");
		    $(".miao_xl").hide();
	  }
  ...
  $(".sppin .hdfk_2_1").click(function(){
	   $(".hdfk_2_1 span").not($("span",$(this))).removeClass("select");
	  $("span",$(this)).addClass("select");
	  $(".hdfk_2_1 input").not($("input",$(this))).attr("checked",false);
	  $("input",$(this)).attr("checked",true);
	  
	  selectShipping($("input",$(this)).get(0));
  });
  
  
  $(".shippingbao").click(function(){
	   $(".shippingbao span").not($("span",$(this))).removeClass("select");
	  $("span",$(this)).addClass("select");
	  $(".shippingbao input").not($("input",$(this))).attr("checked",false);
	  $("input",$(this)).attr("checked",true);
	  var addhtmls=$(".adrhtm",$(this)).html();
	  $(".addselect").html(addhtmls);
	  $(".cadres").removeClass("miaoaon").addClass("miaoaoff");
	  $(".miao_xl").hide();
  });
  ...
  $(".caca").click(function(){
	...
	var recid=$(this).parent().prev().val();
...
function change_fp_address(id) {
  ...
  function drop_goods(id, obj,bjsf) {
    var nobjs = $(obj);
    var itemind = $(".caca").index(nobjs);
    var surobjs=$(obj).parent().parent();
    if (id > 0) {
	 var lsfs = bjsf.ladda();
        $.ajax({
            type: 'POST',
...
function relashCart() {
  ...
  if(id>0){
     ...
     jQuery.ajax({
                ...
                dataType:"json",
                beforeSend:function(XMLHttpRequest){
                $(".cmbloadding",elm.parent().parent().parent()).fadeIn();
                },
                success: function(data){
                   $(".cmbloadding",elm.parent().parent().parent()).fadeOut();
                     switch(data.err){
                        case "update_success":
					
						//elm.val(data.number);
					   
						elm.parent().next().html(data.total_price);
