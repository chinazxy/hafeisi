 ...
 //  $(".cont_personal_box .title1 h1").eq(tz_stpeval).removeClass("select");
  ...
  //  $(".cont_personal_box .title1 h1").eq(2).addClass("select")
   ...
   $("#ECS_INVTYPE").live("change",
    ...
    $(".address_contain").delegate(".dagou,.inuo", "click",
    function(e) {
    ...
    $(".address_contain").delegate(".xyb", "click",
    function(e) {
  ...
  $(".m_haoma").css("border","1px solid red");
    ...
    function initForm() {
        ...
        $(".address_contain #m_selProvince >option").eq(0).attr("selected", "selected");
        $(".address_contain #m_selProvince").trigger("change");

        $(".address_contain #m_selCities >option").eq(0).attr("selected", "selected");
        $(".address_contain #m_selCities").trigger("change");
...
var maddress ='';
    ...
    $(".contiune_address").click(function() {
     ...
     if (hdfk_2val >= 0) {
            ...
            success: function(data) {
                ...
                $(".address_li").bind("click");
                $(".title1 h1").eq(1).addClass("select");
    ...
    function submitEditAdress() {
        ...
        if (wzff_val == 1) {
            ...
            addinfo['ming'] = ming;
            addinfo['surname'] = surname;
            addinfo['address_id'] = address_id;
            addinfo['quhao'] = quhao;
            addinfo['haoma'] = haoma;
            addinfo['province'] = selProvince;
            addinfo['city'] = selCity;
            addinfo['district'] = selDistricts;
        ...
        $.ajax({
            ...
            success: function(data) {
                $(".title1 h1").eq(2).addClass("select");
    ...
    $(".pay_type").click(function() {
        ...
        var message = $(".g_liuk").val();
        var messagelen = getWordLen(message);
        ...
        if (hdfk_2val >= 0) {
            ...
            if (wszf_val >= 0) {
                ...
                if (messagelen <= 140) {
                    ...
                    $(".buymsg").html("留言内容不能超过140个字符!").show();
                    //$(".tishi_pay").html("留言内容不能超过140个字符。").show();   
                }
    ...
    if (id > 0) {
        ...
        $.ajax({
            ...
            beforeSend: function(XMLHttpRequest) {
                ...
                $("img", obj).attr("src", "themes/default/images/load.gif");
                fpchangestate = false;

            },
            success: function(data) {
                // $("img",obj).attr("src","themes/default/images/ca.png");
                switch (data.err) {

                case "delete_s":
                    $('#ECS_ORDERTOTAL').empty().html(data.content);

                    $(".carnum").html(data.nums);
                    $(".cartop,.navtotal").html(data.totp);
                    $(obj).parent().parent().fadeOut(300,
                    function() {
                        relashCart();
                        $(obj).parent().parent().remove();
                        var gxhlen = $(".order_contain .g_xh").length;
                        $(".l_zq").eq(itemind).remove();
                        if (gxhlen == 0) {
