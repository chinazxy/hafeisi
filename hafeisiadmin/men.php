...
/* 页面的缓存ID */
    ...
    $cat = get_cat_info($parent_id);   // 获得分类的相关信息

	if(empty($cat['cat_plot'])){
	 $cat_plot=get_cat_info($pid);
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        $price_grade[0]['url'] = build_uri('product', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    $brands[0]['url'] = build_uri('product', array('cid' => $cat_id, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        foreach ($cat_filter_attr AS $key => $value)
        {
            ...
            if($temp_name = $db->getOne($sql))
            {
                ...
                $sql = "SELECT a.attr_id,count(a.attr_value) as acount, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value,a.color FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
                       " AS g" .
               ...
               $all_attr_list[$key]['filter_count']=count($attr_list);
                ...
                $all_attr_list[$key]['attr_list'][0]['url'] = build_uri('men', array('cid'=>$cat_id,  'filter_attr'=>$temp_arrt_url), $cat['cat_name']);
                ...
                foreach ($attr_list as $k => $v)
                {
                    ...
                    $temp_arrt_url = implode('.', $temp_arrt_url_arr);
					$filter_count=get_filter_goods_count($children,$temp_arrt_url_arr,$cat_filter_attr);
					//echo $cont."<br/>";
					  $all_attr_list[$key]['attr_list'][$temp_key]['gcount'] = $filter_count;
                    $all_attr_list[$key]['attr_list'][$temp_key]['acount'] = $v['acount'];
					
                    $all_attr_list[$key]['attr_list'][$temp_key]['attr_value'] = $v['attr_value'];
					     $all_attr_list[$key]['attr_list'][$temp_key]['color'] = $v['color'];
                    $all_attr_list[$key]['attr_list'][$temp_key]['url'] = build_uri('men', array('cid'=>$cat_id,'filter_attr'=>$temp_arrt_url), $cat['cat_name']);
...
$smarty->assign("cateinfo",getCatecoryInfo($cat_id));
$smarty->assign("cat",$cat);
    ...
    $smarty->assign('data_dir',    DATA_DIR);
	 $smarty->assign('catelist',    $catelist);
	  ...
	  $smarty->assign('catelen',    count($catelist));
    ...
    foreach($goodslist as $k=>$v){
		...
		$colorfirst=array();
		 for($t=0;$t<count($properties['spe']);$t++){
		 
		  if($properties['spe'][$t]['name']=="颜色"){
		    
			   
	
			  $colorfirst=$properties['spe'][$t]['values'];
   ...
   $nowpage=$page;
    ...
    $smarty->assign('pageItem',       $pageItem);
	...
	$smarty->assign("statea",$states);
  $smarty->assign('hasd',  "d");  

    $smarty->assign("nowpage",$nowpage);
		  $smarty->assign("pid",$pid);	
	$smarty->assign("filter_attr_str",$filter_attr_str);
$ads = array('num'=>'1','id'=>'1');//广告数量,广告位ID
$banner =insert_ads($ads);
$smarty->assign("banner",$banner); 
    assign_pager('men',  $cat_id, $count, $size, $sort, $order, $page, '', $price_min, $price_max, $display, $filter_attr_str); // 分页
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc,cat_endesc,cat_addtype,des_title,des_entitle, style, grade, filter_attr, parent_id,cat_plot,cat_enname FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = ".$cat_id);
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name,g.brand_id, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['no_farmart_shop_price']       =$row['shop_price'];
        $arr[$row['goods_id']]['type']             = $row['goods_type'];
			$arr[$row['goods_id']]['goods_crank']         = get_goods_comment_ranks($row['goods_id']);
...
function get_filter_goods_count($children,$filter_attr,$cat_filter_attr){
 $file_w  = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
        ...
        if (!empty($filter_attr))
        {
            ...
            foreach ($filter_attr AS $k => $v)                      // 查出符合所有筛选属性条件的商品id */
            {
                ...
                if ($v != 0)
                {
                   ...
                   $fext .= ' AND ' . db_create_in($ext_group_goods, 'g.goods_id');
                }
            }
        }
		
		//echo 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('goods') . " AS g WHERE $where $ext";
         return $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('goods') . " AS g WHERE $file_w $fext");
}
/**
 * 获得分类下的商品总数
 *
...
function getchildCategoty($pid,$filter_attr_str){

  $sql="SELECT * FROM ".$GLOBALS['ecs']->table('category')." where parent_id=".$pid." order by sort_order asc";
  
  $catlist=$GLOBALS['db']->getAll($sql);
 
 // for($i=0;$i<count($catlist);$i++){
  
   //  $catlist[$i]['url']=build_uri('product', array('cid'=>$catlist[$i]['cat_id'],'filter_attr'=>$filter_attr_str), $catlist[$i]['cat_name']);
