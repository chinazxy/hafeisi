...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name,goods_type, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount)
    VALUES ('$type_name',
	        '$_POST[goods_type]',
            '$_POST[type_money]',
            '$send_startdate',
...
if ($_REQUEST['act'] == 'bonus_list')
{
    ...
    $smarty->assign('action_link2',   array('href' => 'bonus.php?act=import&bonus_type='.$_REQUEST['bonus_type'], 'text' =>"导入优惠卷"));
    ...
    elseif ($bonus_type['send_type'] == SEND_BY_USER)
    {
     ...
     $smarty->assign('bonus_type', $_REQUEST['bonus_type']);
    $smarty->assign('bonus_goods_type',   $bonus_goods_type);
...
if($_REQUEST['act']=="edit_bouns"){
	$bounsid=$_REQUEST['bonus_id'];
	$bonus_type=$_REQUEST['bonus_type'];
	if($bounsid>0){
		 
	      $bounsinfo=$db->getRow("SELECT * from ".$ecs->table("user_bonus")." where bonus_id=".$bounsid);
		  
		  if($bounsinfo['user_id']>0){
			  
			  $userinfo=$db->getRow("SELECT * from ".$ecs->table("users")." where user_id=".$bounsinfo['user_id']);
			  $bounsinfo['user_email']=$userinfo['email'];
			   $bounsinfo['order_sn']=$db->getOne("SELECT order_sn from ".$ecs->table("order_info")." where order_id=".$bounsinfo['order_id']);
		  }
		    if($bounsinfo['used_time']>0){
				
				  $bounsinfo['use_time_formart']=date("Y-m-d H:i:s",$bounsinfo['used_time']);
			}
		      if($bounsinfo['start_time']>0){
				
				 	    $bounsinfo['use_start_time']=date("Y-m-d H:i:s",$bounsinfo['start_time']);
			}
	
				      if($bounsinfo['end_time']>0){
				
				 	   	 $bounsinfo['use_end_time']=date("Y-m-d H:i:s",$bounsinfo['end_time']);
			}
		
          $smarty->assign('bounsinfo', $bounsinfo);	
   $smarty->assign('bonus_type',$bonus_type);	
		 $smarty->assign('form_act',"act_bouns_edit");  
		  $smarty->display('bonus_edit.htm');	
	}else{
		
		  $link[0]['text'] = "优惠卷列表";
		  
		    $link[0]['href'] = 'bonus.php?act=bonus_list&bonus_type=' . $bonus_type;
		    sys_msg("无效的ID", 0, $link);
	}
...
if($_REQUEST['act']=="import"){
  ...
  $bonus_type=$_REQUEST['bonus_type'];
 $smarty->assign('bonus_type',$bonus_type);  
  $smarty->assign('form_act',"act_bouns_import");  
  $smarty->display('bonus_import.htm');
}


if($_REQUEST['act']=="act_bouns_import"){
	ini_set('memory_limit','-1');
  require(ROOT_PATH . 'includes/PHPExcel/PHPExcel.php');
 	require_once ROOT_PATH . 'includes/PHPExcel/PHPExcel/IOFactory.php';
 
  require_once ROOT_PATH . 'includes/PHPExcel/PHPExcel/Reader/Excel5.php';
  ...
  $filename =$arrstr.substr($_FILES['file']['name'], strpos($_FILES['file']['name'], '.'));
  $path= ROOT_PATH.DATA_DIR. "/bonus/".$filename;
  ...
  if($_FILES['file']['error']==0){
   ...
   if (!move_upload_file($_FILES['file']['tmp_name'],$path))
    {
        return false;
    }else{
...
$bonus=trim($objPHPExcel->getActiveSheet()->getCell("D".$j)->getValue());
...
if($price=="500"){
	...
	$sql="INSERT INTO ".$ecs->table("user_bonus")."(bonus_id,bonus_type_id,bonus_sn) VALUES(NULL,'3','".$bonus."')";	
 $db->query($sql);	
}

if($price=="100"){
	
	$sql="INSERT INTO ".$ecs->table("user_bonus")."(bonus_id,bonus_type_id,bonus_sn) VALUES(NULL,'4','".$bonus."')";	
 $db->query($sql);	
}
...
if($price=="50"){
	...
	$sql="INSERT INTO ".$ecs->table("user_bonus")."(bonus_id,bonus_type_id,bonus_sn) VALUES(NULL,'5','".$bonus."')";	
 $db->query($sql);	
}
...
if($price=="30"){
	...
	$sql="INSERT INTO ".$ecs->table("user_bonus")."(bonus_id,bonus_type_id,bonus_sn) VALUES(NULL,'6','".$bonus."')";	
 $db->query($sql);	
}
         ...
         $link[0]['text'] = "优惠卷列表";
		    ...
		    $link[0]['href'] = 'bonus.php?act=import&bonus_type=' . $bonus_type;
		    sys_msg("请上传有效的文件", 0, $link);
  }  	
  }else{
...
if($_REQUEST['act']=="act_bouns_edit"){
	...
	if($bounsid>0){
		$startime=strtotime($_REQUEST['use_start_date']);
	   $endtime=strtotime($_REQUEST['use_end_date']);

	   $sql="UPDATE ".$ecs->table("user_bonus")." SET start_time='".$startime."',end_time='".$endtime."' where bonus_id=".$bounsid;
	 
	   $state=$db->query($sql);
	   if($state){
			  $link[0]['text'] = "返回";
		  
		     $link[0]['href'] = 'bonus.php?act=edit_bouns&bonus_type=' . $bonus_type."&bonus_id=".$bounsid;
		    sys_msg("修改成功", 0, $link);   
...
if ($_REQUEST['act'] == 'query_bonus')
{
    ...
    elseif ($bonus_type['send_type'] == SEND_BY_USER)
    {
     ...
     $smarty->assign('bonus_goods_type',   $_REQUEST['bonus_type']);
    $smarty->assign('bonus_list',   $list['item']);
    $smarty->assign('filter',       $list['filter']);
    $smarty->assign('record_count', $list['record_count']);
    $smarty->assign('page_count',   $list['page_count']);
...
function get_bonus_list()
{
    ...
    $filter['bonus_goods_type']=$GLOBALS['db']->getOne("SELECT goods_type from ".$GLOBALS['ecs']->table("bonus_type")." where type_id=".$_REQUEST['bonus_type']);
	$where = empty($filter['bonus_type']) ? '' : " WHERE ub.bonus_type_id='$filter[bonus_type]'";
	if(!empty($where)){
		
    if(!empty($filter['keywords'])){
	$where.=" and  ub.bonus_sn='".$filter['keywords']."'";	
	}
	 
	}else{
		
		
    $where.="where ub.bonus_sn='".$filter['keywords']."'";	
      }
	
 
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('user_bonus')." as ub ". $where;
