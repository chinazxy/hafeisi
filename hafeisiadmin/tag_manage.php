...
elseif($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if($is_add)
    {
        $sql = "select goods_id,xingou_id,goods_name from ". $ecs->table('goods')." where is_delete=0 and is_on_sale=1";
        $res = $db->getAll($sql);
        $tag['goods'] = $res;
    ...
    else
    {
        ...
        $tag['goods'] = $res;
        $tag_id = $_GET['id'];
        $sqll = "select goods_id,tag_words from " . $ecs->table('tag') . " where tag_id=$tag_id";
        $goods_id = $db->getRow($sqll);
        $tag['tag_words'] = $goods_id['tag_words'];
        $smarty->assign('tag_id' , $tag_id);
        $smarty->assign('tag_words' , $tag['tag_words']);
        $smarty->assign('goods_id' , $goods_id['goods_id']);
        $smarty->assign('ur_here',      $_LANG['tag_edit']);
    }
    $smarty->assign('tag', $tag['goods']);
...
//var_dump($_POST);exit;
    ...
    $sql = "select xingou_id from ". $ecs->table('goods') . " where goods_id=$goods_id";
    $xingou_id = $db->getOne($sql);
    if ($goods_id <= 0)
    {
    ...
    if($is_insert)
    {
        ...
        $sql = 'INSERT INTO ' . $ecs->table('tag') . '(goods_id, xingou_id, tag_words)' .
               " VALUES('$goods_id','$xingou_id', '$tag_words')";
        $db->query($sql);
    ...
    else
    {
        ...
        edit_tag($tag_words, $id, $goods_id, $xingou_id);
...
function tag_is_only($name, $tag_id = '', $goods_id)
{
    ...
    {
        $db = $GLOBALS['db'];
        $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('tag') . " WHERE tag_id = '$tag_id' and goods_id=$goods_id and tag_words='$name'";
 ...
 * @return void
 */
function edit_tag($name, $id, $goods_id = '',$xingou_id)
{
    $db = $GLOBALS['db'];
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('tag') . " SET tag_words = '$name',goods_id=$goods_id,xingou_id=$xingou_id where tag_id=$id";
