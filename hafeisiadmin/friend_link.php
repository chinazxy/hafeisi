...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("link_name", $link_name) == 0)
    {
        ...
        {
			handle_update_image($lastid,$_FILES['link_img'],158,78);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ((isset($_FILES['link_img']['error']) && $_FILES['link_img']['error'] == 0) || (!isset($_FILES['link_img']['error']) && isset($_FILES['link_img']['tmp_name']) && $_FILES['link_img']['tmp_name'] != 'none'))
    {
            $logo = $db->getRow("SELECT link_logo,link_thump_logo FROM " .$ecs->table('friend_link'). " WHERE link_id = '$id'");
       
             $img_name = basename($logo['link_logo']);
			 $img_thump_name = basename($logo['link_thump_logo']);
            @unlink(ROOT_PATH . DATA_DIR . '/afficheimg/' . $img_name);
         @unlink(ROOT_PATH . DATA_DIR . '/afficheimg/' . $img_thump_name);
   
		
		handle_update_image($id,$_FILES['link_img'],158,78);
...
elseif ($_REQUEST['act'] == 'remove')
{
	...
	$link_thump_logo = $exc->get_name($id, "link_thump_logo");
  ...
  //  if ((strpos($link_logo, 'http://') === false) && (strpos($link_logo, 'https://') === false))
   // {
        ...
        @unlink(ROOT_PATH. DATA_DIR . '/afficheimg/'.$img_name);
		 ...
		 $img_name = basename($link_thump_logo);
        @unlink(ROOT_PATH. DATA_DIR . '/afficheimg/'.$img_name);
   // }
...
$orgImg=DATA_DIR . "/afficheimg/".$filename;


$path_m    = DATA_DIR . "/afficheimg/"."m_".$filename;



$thumb_url = $image->make_thumb($image_files['tmp_name'],$articel_width,$articel_height,$path.DATA_DIR."/afficheimg/"); 


$thumb_url = DATA_DIR ."/afficheimg/".is_string($thumb_url) ? $thumb_url : '';
      ...
      $img_info = getimagesize($path.$thumb_url);
                   ...
                   if (isset($img_info[2])) {
					...
					for ($y = 0; $y < $rs['img_height'] ; $y++) {
                     ...
                     imagejpeg($im,$path.DATA_DIR . "/afficheimg/".$gray, 100);


$sql = "UPDATE ".$GLOBALS['ecs']->table('friend_link')." SET link_logo='$thumb_url',link_thump_logo='$gray' where link_id='".$news_id."' ";
