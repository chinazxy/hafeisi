...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here',    '主推图管理');
    $smarty->assign('action_link', array('text' =>'主推图添加', 'href' => 'homes.php?act=add'));
    ...
    $smarty->display('zhutuilist.htm');
}elseif($_REQUEST['act'] == 'add'){

    $smarty->assign('action',        'add');
	   $smarty->assign('form_act', 'insert');
 $smarty->display('zhutui.htm');
}elseif ($_REQUEST['act'] == 'insert')
{
	...
	$addtime=time();
$sql = "INSERT INTO ".$GLOBALS['ecs']->table('zhutui')."(title,url,images,resort,addtime,isshow,thump_o,images_s,thump_o_s) ".
            "VALUES ('$title','$url','$image_p','$resort','$addtime','$isshow','$image_p_m','$image_s','$image_s_m')";
    ...
    $link[1]['text'] = "返回列表";
    $link[1]['href'] = 'homes.php?act=list';

    $link[2]['text'] = "继续添加";
    $link[2]['href'] = 'homes.php?act=add';
...
}elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT * FROM " .$ecs->table('zhutui'). " WHERE id='".intval($_REQUEST['id'])."'";
    $ads_arr = $db->getRow($sql);

    $ads_arr['title'] = htmlspecialchars($ads_arr['title']);


    $smarty->assign('ur_here',       "主推图片修改");
    $smarty->assign('action_link',   array('href' => 'homes.php?act=list', 'text' =>'主推图片列表'));
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    handle_update_gallery_image($id,$_FILES['file'],$_FILES['file_s'],$title,$url,$resort,$isshow);
   /* 记录管理员操作 */
   ...
   /* 提示信息 */
   $href[] = array('text' => '返回列表', 'href' => 'homes.php?act=list');
...
$id=$_GET['id'];
$din=$_GET['din'];
$sql="SELECT * from ".$ecs->table('zhutui')." where id=".$id;
...
$sql="Update  ".$ecs->table('zhutui')." SET images='',thump_o='' where id=".$id;
}else{
...
$sql="Update  ".$ecs->table('zhutui')." SET images_s='',thump_o_s='' where id=".$id;
}
$state=$db->query($sql);
...
if($state){
 ...
 $links[0]['text']    ="返回主推管理页面";
  $links[0]['href']    = 'homes.php?act=edit&id='.$id;
...
$image=$db->getRow($sql);
    ...
    $sql="DELETE  FROM  ".$ecs->table('zhutui'). " where id=$id";

    $state=$db->query($sql);
	

  $url = 'homes.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
}elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('zhutuilist.htm'), '',
...
function get_lunbolist()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('zhutui'). ' AS ad ' . $where;
    ...
    $arr = array();
    $sql = 'SELECT *'.
            'FROM ' .$GLOBALS['ecs']->table('zhutui'). 'AS ad ' .
...
function handle_update_gallery_image($ids,$image_files,$image_files_s,$image_title,$image_url,$image_sort,$isshow)
...
{ 
	...
	$image_s='';
	$image_s_m='';
	$sql="SELECT * from ".$GLOBALS['ecs']->table('zhutui')." where id=".$ids;
