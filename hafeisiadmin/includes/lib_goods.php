 ...
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls,$width,$height)
{
    /* 是否处理缩略图 */
    $proc_thumb = (isset($GLOBALS['shop_id']) && $GLOBALS['shop_id'] > 0)? false : true;
	if(count($image_descs)>0){
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            if ($proc_thumb)
            {
                $thumb_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key], $width,  $height);
				$thumb_url_t = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key],120,120);
                $thumb_url = is_string($thumb_url) ? $thumb_url : '';
				   $thumb_url_t = is_string($thumb_url_t) ? $thumb_url_t : '';
            }
            ...
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url,thumb_url_t, img_original) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url','$thumb_url_t', '$img_original')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
        ...
        elseif (!empty($image_urls[$key]) && ($image_urls[$key] != $GLOBALS['_LANG']['img_file']) && ($image_urls[$key] != 'http://') && copy(trim($image_urls[$key]), ROOT_PATH . 'temp/' . basename($image_urls[$key])))
        {
            ...
            if ($proc_thumb)
            {
                $thumb_url = $GLOBALS['image']->make_thumb($down_img, $width,$height);
					$thumb_url_t = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key],120,120);
                $thumb_url = is_string($thumb_url) ? $thumb_url : '';
                $thumb_url = reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb');
				
				 $thumb_url_t = is_string($thumb_url_t) ? $thumb_url_t : '';
                $thumb_url_t = reformat_image_name('gallery_thumb', $goods_id, $thumb_url_t, 'thumb');
            }
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type,v.color, a.attr_values,a.attr_envalues,v.attr_usa_price,v.attr_value,a.attr_color,v.attr_envalue,v.attr_price,v.attr_thumb,v.attr_thumps,v.attr_org,v.attr_left_thumb,v.attr_left_thumps,v.attr_left_org,v.frame_des,v.glass_des,v.xinghao,v.frame_endes,v.xinghao,v.glass_endes ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        {
			if($val['attr_name']=="颜色筛选属性"){
				
				   $html .= '<select id="colorfilter"  onchange="selectVal(this);"  name="attr_value_list[]">';
			}else{
				
			   $html .= '<select onchange="selectOth(this);"  name="attr_value_list[]">';	
			}
         
            $html .= '<option value="" colorval="">' .$GLOBALS['_LANG']['select_please']. '</option>';
...
$aco = explode("\n", $val['attr_color']);
            foreach ($attr_values AS $k=>$opt)
            {
                ...
                $html   .= ($val['attr_value'] != $opt) ?
                    '<option value="' . $opt . '" colorval="'.$aco[$k].'">' . $opt . '</option>' :
                    '<option value="' . $opt . '" selected="selected"  colorval="'.$aco[$k].'">' . $opt . '</option>';
            }
            $html .= '</select> ';
			
			
			  $html .= '<select name="attr_envalue_list[]">';
            $html .= '<option value="">' .$GLOBALS['_LANG']['select_please']. '</option>';

            $attr_envalues = explode("\n", $val['attr_envalues']);

            foreach ($attr_envalues AS $opt)
            {
                $opt    = trim(htmlspecialchars($opt));

                $html   .= ($val['attr_envalue'] != $opt) ?
        ...
        if($val['attr_name']=="颜色"){
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2  || $val['attr_type'] == 3) ?
            '英文名：<input name="attr_envalue_list[]" type="text" value="' .htmlspecialchars($val['attr_envalue']). '" size="40" />'.$GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />属性美元价：<input type="text" name="attr_usa_price_list[]" value="' . $val['attr_usa_price'] . '" size="5" maxlength="10" />' :
            ' <input type="hidden" name="attr_price_list[]" value="0" />';
          if ($val['attr_type'] == 1){
		  $html .= '颜色值：<input type="text" name="colors[]" value="'.$val['color'].'" /><input type="hidden" value="'.$val['color'].'" name="colorval[]"  >
		  对应镜框描述(中)：<input type="text" name="frame_des[]" value="'.$val['frame_des'].'" /><input type="hidden" value="'.$val['frame_des'].'" name="frame_desval[]"  >对应镜框描述(英)：<input type="text" name="frame_endes[]" value="'.$val['frame_endes'].'" /><input type="hidden" value="'.$val['frame_endes'].'" name="frame_endesval[]"  >对应镜片描述(中)：<input type="text" name="glass_des[]" value="'.$val['glass_des'].'" /><input type="hidden" value="'.$val['glass_des'].'" name="glass_desval[]"  >对应镜片描述(英)：<input type="text" name="glass_endess[]" value="'.$val['glass_endes'].'" /><input type="hidden" value="'.$val['glass_endes'].'" name="glass_endesval[]"  ><br/>产品色号<input type="text" name="xinghao[]" value="'.$val['xinghao'].'" /><input type="hidden" value="'.$val['xinghao'].'" name="xinghaoval[]"  >正面图：<input type="file" name="attr_image_list[]" />';
		 if(!empty($val['attr_thumb'])){
		  
		   $html.='对应正面图：<img src="../'.$val['attr_thumb'].'" width="50px" height="50px">';
		  
		  }
		  
		   $html .= '<input type="hidden" name="img[]" value="' . $val['attr_org'] . '" /><input type="hidden" name="attr_thumps[]" value="' . $val['attr_thumps'] . '" /><input type="hidden" name="imgthmp[]" value="' . $val['attr_thumb'] . '" />
		  侧面图：<input type="file" name="attr_image_left_list[]" />';
		  	  	  if(!empty($val['attr_left_thumb'])){
		  
		   $html.='对应侧面图：<img src="../'.$val['attr_left_thumb'].'" width="50px" height="50px">';
		  
		  }
		   $html .= '<input type="hidden" name="imgleft[]" value="' . $val['attr_left_org'] . '" /><input type="hidden" name="attr_left_thumps[]" value="' . $val['attr_left_thumps'] . '" /><input type="hidden" name="imgleftthmp[]" value="' . $val['attr_left_thumb'] . '" />';
		
	
		
		  }
		  }else if($val['attr_name']=="尺寸"){
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2  || $val['attr_type'] == 3) ?
            $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />' :
            ' <input type="hidden" name="attr_price_list[]" value="0" />';
          if ($val['attr_type'] == 1){
		  $html .= '尺寸值:<input type="text" name="colors[]" value="'.$val['color'].'" /><input type="hidden" value="'.$val['color'].'" name="colorval[]"  ><input style="display:none;" type="file" name="attr_image_list[]" /><input type="hidden" name="img[]" value="' . $val['attr_org'] . '" /><input type="hidden" name="attr_thumps[]" value="' . $val['attr_thumps'] . '" /><input type="hidden" name="imgthmp[]" value="' . $val['attr_thumb'] . '" />';
		  if(!empty($val['attr_thumb'])){
		  
		   $html.='<img style="display:none;" src="../'.$val['attr_thumb'].'" width="50px" height="50px">';
		  
		  }
		  }
		  }else if($val['attr_name']=="颜色筛选属性"){
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2  || $val['attr_type'] == 3) ?
            ' <input style="display:none;" type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />' :
            ' <input type="hidden" name="attr_price_list[]" value="0" />';
          if ($val['attr_type'] == 1){
		  $html .= '<input type="text"  style="display:none;" name="colors[]" value="'.$val['color'].'" /><input type="hidden" value="'.$val['color'].'" name="colorval[]"  ><input style="display:none;" type="file" name="attr_image_list[]" /><input type="hidden" name="img[]" value="' . $val['attr_org'] . '" /><input type="hidden" name="attr_thumps[]" value="' . $val['attr_thumps'] . '" /><input type="hidden" name="imgthmp[]" value="' . $val['attr_thumb'] . '" />';
		  if(!empty($val['attr_thumb'])){
		   ...
		   $html.='<img style="display:none;" src="../'.$val['attr_thumb'].'" width="50px" height="50px">';
            ...
            ' <input type="hidden" name="attr_price_list[]" value="0" />';
          if ($val['attr_type'] == 1){
		  $html .= '<input style="display:none;" type="text" name="colors[]" value="'.$val['color'].'" /><input type="hidden" value="'.$val['color'].'" name="colorval[]"  ><input style="display:none;" type="file" name="attr_image_list[]" /><input type="hidden" name="img[]" value="' . $val['attr_org'] . '" /><input type="hidden" name="attr_thumps[]" value="' . $val['attr_thumps'] . '" /><input type="hidden" name="imgthmp[]" value="' . $val['attr_thumb'] . '" />';
		  if(!empty($val['attr_thumb'])){
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn,goods_thumb, shop_price, is_on_sale, is_best, is_new, is_hot, is_discount,sort_order, goods_number, integral, " .
...
function get_goods_specifications_list($goods_id)
{
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            ...
            WHERE goods_id = '$goods_id'
            AND a.attr_type = 1 and a.attr_name='颜色'
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number,product_sku
...
function check_product_sku_exist($product_sku, $product_id = 0)
{
