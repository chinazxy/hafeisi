...
if ($_REQUEST['act'] == '')
{
    ...
    $smarty->assign('ur_here',       "导出咨询信息");
    $smarty->assign('action_link',   array('href' => 'zx.php?act=list&pid=1', 'text' =>'咨询列表'));
    $smarty->assign('form_act',      'update');
    $smarty->assign('action',        'edit');

    assign_query_info();
    $smarty->display('zxexcel.htm');
}elseif($_REQUEST['act']=="import_excel"){

 $starttime=$_POST['start_time'];
 
 $endtime=$_POST['end_time'];
 $where=" where ad.zhixun_type = 0";
...
$savetime="咨询Excel_".date('Y-m-d',time());
