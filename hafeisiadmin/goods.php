...
require(dirname(__FILE__) . '/efast/product_stock.php');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        if ($goods['goods_weight'] > 0)
        {
		 ...
		 if (!empty($goods['goods_size']))
        {
            //$goods['goods_brief'] = trim_right($goods['goods_brief']);
            $goods['goods_size'] = $goods['goods_size'];
        }
		
				 if (!empty($goods['goods_material']))
        {
            //$goods['goods_brief'] = trim_right($goods['goods_brief']);
            $goods['goods_material'] = $goods['goods_material'];
        }
		
		if (!empty($goods['goods_xijie']))
        {
            //$goods['goods_brief'] = trim_right($goods['goods_brief']);
            $goods['goods_xijie'] = $goods['goods_xijie'];
        }
        ...
        if ($_REQUEST['act'] == 'copy')
        {
        	...
        	$sql="SELECT * FROM ".$ecs->table('goods_color')." where goods_id=".$goods['goods_id']." order by resort asc";
        ...
        if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 0))
        {
            foreach ($img_list as $key => $gallery_img)
            {
				...
				$gallery_img[$key]['thumb_url_t'] = get_image_path($gallery_img['goods_id'], $gallery_img['img_original'], true, 'gallery');
            }
        }
        else
        {
            foreach ($img_list as $key => $gallery_img)
            {
                $gallery_img[$key]['thumb_url'] = '../' . (empty($gallery_img['thumb_url']) ? $gallery_img['img_url'] : $gallery_img['thumb_url']);
				 $gallery_img[$key]['thumb_url_t'] = '../' . (empty($gallery_img['thumb_url_t']) ? $gallery_img['thumb_url_t'] : $gallery_img['thumb_url_t']);
            }
        }
    }
    ...
    /* 创建 html editor */
	 ...
	 create_html_editor2('size_des', $goods['size_des']);
     create_html_editor3('size_endes', $goods['size_endes']);
	   create_html_editor4('glass_dess', $goods['glass_des']);
	        create_html_editor5('glass_endes',$goods['glass_endes']);
			
				   create_html_editor6('good_des', $goods['good_des']);
	        create_html_editor7('good_endes',$goods['good_endes']);
			
			
      create_html_editor8('goods_brief', $goods['goods_brief']);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (isset($_FILES['goods_img']['error'])) // php 4.2 版本才支持 error
    {
        ...
        // 商品背景图片
        if ($_FILES['back_goods_img']['error'] == 0)
        {
            if (!$image->check_img_type($_FILES['back_goods_img']['type']))
            {
                sys_msg($_LANG['invalid_goods_img'], 1, array(), false);
            }else{
                $back_goods_img   = $image->upload_image($_FILES['back_goods_img']); // 原始图片
            }
        }
        elseif ($_FILES['back_goods_img']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['goods_img_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['back_goods_img']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['goods_img_too_big'], $htm_maxsize), 1, array(), false);
        }

        // 手机详情图片
        if ($_FILES['mobile_detail_img']['error'] == 0)
        {
            if (!$image->check_img_type($_FILES['mobile_detail_img']['type']))
            {
                sys_msg($_LANG['invalid_goods_img'], 1, array(), false);
            }else{
                $mobile_detail_img   = $image->upload_image($_FILES['mobile_detail_img']); // 原始图片
            }
        }
        elseif ($_FILES['mobile_detail_img']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['goods_img_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['mobile_detail_img']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['goods_img_too_big'], $htm_maxsize), 1, array(), false);
        }
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        ...
        {
            /* 删除原来的图片文件 */
            $sql = "SELECT goods_thumb, goods_img, original_img,goods_photo_thumb " .
        ...
        if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']) || $is_url_goods_img)
        {
            ...
            if ($_CFG['auto_generate_gallery'])
            {
                ...
                {
                    $gallery_thumb = $image->make_thumb('../' . $img, 880,566);
    ...
    else
    {
        ...
        if ($proc_thumb && isset($_POST['auto_thumb']) && !empty($original_img))
        {
            ...
            if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0)
            {
                ...
                $goods_thumb = $image->make_thumb('../' . $original_img, 280, 180);
				$goods_photo_thumb= $image->make_thumb('../' . $original_img, 880, 566);
                if ($goods_thumb === false)
                {
                    sys_msg($image->error_msg(), 1, array(), false);
                }
    ...
    $usa_price = !empty($_POST['usa_price']) ? $_POST['usa_price'] : 0;
    ...
    $goods_enname = isset($_POST['goods_enname']) ? $_POST['goods_enname'] : "";
    $goods_name_style = $_POST['goods_name_color'] . '+' . $_POST['goods_name_style'];
	$instructions = isset($_POST['instructions']) ? $_POST['instructions'] : "";
	$goods_sku=isset($_POST['goods_sku'])?trim($_POST['goods_sku']):"";
    $sub_title = isset($_POST['sub_title']) ? $_POST['sub_title'] : "";
    $xingou_id = isset($_POST['xingou_id']) ? intval($_POST['xingou_id']) : 0;
    $catgory_id = empty($_POST['cat_id']) ? '' : intval($_POST['cat_id']);
    $brand_id = empty($_POST['brand_id']) ? '' : intval($_POST['brand_id']);
	$tmail_url=$_POST['tmail_url'];
	...
	$back_goods_img = !empty($back_goods_img) ? $back_goods_img : "";
	$goods_size=$_POST['goods_size'];
	
	$goodscolor=$_POST['colorval'];
	
	$colorprice=$_POST['color_goods_price'];
	
	$colorresort=$_POST['colorresort'];
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (mobile_detail_img,back_goods_img,description,xingou_id,sub_title,instructions,goods_name,goods_enname, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img,goods_photo_thumb, keywords, goods_brief, " .
                    "seller_note,goods_size, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_discount," .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,xingji, suppliers_id, goods_material,goods_xijie,otherinfo,tmail_url,goods_sku,size_des,size_endes,glass_des,glass_endes,good_des,good_endes,usa_price)" .
                "VALUES ('$mobile_detail_img', '$back_goods_img', '$description', '$xingou_id', '$sub_title', '$instructions', '$_POST[goods_name]', '$goods_enname', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img','$goods_photo_thumb', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]','$_POST[goods_size]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot','$is_discount', '$is_on_sale', '$is_alone_sale', '$is_shipping', ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$_POST[xingji]', '$suppliers_id','$_POST[goods_material]','$_POST[goods_xijie]','$_POST[otherinfo]','$_POST[tmail_url]','$goods_sku','$_POST[size_des]','$_POST[size_endes]','$_POST[glass_dess]','$_POST[glass_endes]','$_POST[good_des]','$_POST[good_endes]','$usa_price')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (mobile_detail_img,back_goods_img,description,xingou_id,instructions,goods_name,goods_enname, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img,goods_photo_thumb, keywords, goods_brief, " .
                    "seller_note,goods_size,xingji, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot,is_discount, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,goods_material,goods_xijie,otherinfo,tmail_url,goods_sku,size_des,size_endes,glass_des,glass_endes,good_des,good_endes,usa_price)" .
                "VALUES ('$mobile_detail_img', '$back_goods_img', '$description', '$xingou_id', '$sub_title', '$instructions', '$_POST[goods_name]','$goods_enname', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img','$goods_photo_thumb', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]','$_POST[goods_size]','$_POST[xingji]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot','$is_discount', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$_POST[goods_material]','$_POST[goods_xijie]','$_POST[otherinfo]','$_POST[tmail_url]',$goods_sku,'$_POST[size_des]','$_POST[size_endes]','$_POST[glass_dess]','$_POST[glass_endes]','$_POST[good_des]','$_POST[good_endes]','$usa_price')";
        }
    }
    else
    {
        ...
        if ($back_goods_img)
        {
            $sql .= "back_goods_img = '$back_goods_img', ";
        }
        if ($mobile_detail_img)
        {
            $sql .= "mobile_detail_img = '$mobile_detail_img', ";
        }
        if ($goods_thumb)
        {
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
                ...
                "goods_desc = '$_POST[goods_desc]', " .
				   "goods_size = '$_POST[goods_size]', " .
				      "goods_material = '$_POST[goods_material]', " .
				      "xingji = '$_POST[xingji]'," .
                "last_update = '". gmtime() ."',".
				 "goods_xijie = '$_POST[goods_xijie]',".
				  "otherinfo= '$_POST[otherinfo]',".
				    "tmail_url= '$_POST[tmail_url]',".
					   "goods_sku= '$goods_sku',".
					    "size_des= '$_POST[size_des]', " .
						 "size_endes= '$_POST[size_endes]', " .
						  "glass_des= '$_POST[glass_dess]', " .
						   "glass_endes= '$_POST[glass_endes]', " .
						    "good_des= '$_POST[good_des]', " .
							 "good_endes= '$_POST[good_endes]', " .
...
//echo $sql;exit();
	...
	if(count($goodscolor)>0){
	  
	  for($i=0;$i<count($goodscolor);$i++){
	    if($goodscolor[$i]>0 && $goods_id>0 && $colorprice[$i]>0){
		$sql2="INSERT INTO  ".$ecs->table('goods_color')." (color,price,resort,goods_id) VALUES(".$goodscolor[$i].",".$colorprice[$i].",".$colorresort[$i].",".$goods_id.")";
	...
	$editcolorval=$_POST['editcolorval'];
	
	$edit_color_goods_price=$_POST['edit_color_goods_price'];
	
	$edit_colorresort=$_POST['edit_colorresort'];
	if(count($editid>0)){
	
	  for($i=0;$i<count($editid);$i++){
	  
	   $sql="UPDATE " . $ecs->table('goods_color') . " set color='".$editcolorval[$i]."', price='".$edit_color_goods_price[$i]."',resort='".$edit_colorresort[$i]."' where id=".$editid[$i];
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
		...
		//print_r($_FILES['attr_image_list']);
		
		 $attr_image = $_FILES['attr_image_list'];
        ...
        if(isset($_POST['attr_id_list']))
        {
           ...
           if(!$_FILES['attr_image_list']['error'] > 0){
          ...
          if($_FILES['attr_image_list']['name'][$key]==''){

          if($_POST['img'][$key]!=''){

            $attr_img=$_POST['img'][$key];
			$thumb_url=$_POST['imgthmp'][$key];
			$thumbs_url=$_POST['attr_thumps'][$key];
			if($_POST['colors'][$key]!=''){
			$color=$_POST['colors'][$key];
			}else{
			
		    $color=$_POST['colorval'][$key];
			}
			
					if($_POST['frame_des'][$key]!=''){
			$frame_des=$_POST['frame_des'][$key];
			}else{
			
		    $frame_des=$_POST['frame_desval'][$key];
			}
			if($_POST['xinghao'][$key]!=''){
			$xinghao=$_POST['xinghao'][$key];
			}else{
			
		    $xinghao=$_POST['xinghaoval'][$key];
			}
			
						if($_POST['frame_endes'][$key]!=''){
			$frame_endes=$_POST['frame_endes'][$key];
			}else{
			
		    $frame_endes=$_POST['frame_endesval'][$key];
			}
			
				
			if($_POST['glass_des'][$key]!=''){
			$glass_des=$_POST['glass_des'][$key];
			}else{
			
		    $glass_des=$_POST['glass_desval'][$key];
			}
			
					if($_POST['glass_endess'][$key]!=''){
			$glass_endes=$_POST['glass_endess'][$key];
			}else{
			
		    $glass_endes=$_POST['glass_endesval'][$key];
			}
        ...
        //  $aa=time().$_FILES['attr_image_list']['name'][$key];
            $color=$_POST['colors'][$key];
		     $frame_des=$_POST['frame_des'][$key];
			 $xinghao=$_POST['xinghao'][$key];
			...
			$frame_endes=$_POST['frame_endes'][$key];
				$glass_endes=$_POST['glass_endess'][$key];
          $tempname=$_FILES["attr_image_list"]["tmp_name"][$key];
$path= ROOT_PATH;
$image = new cls_image();
$filename =$image->random_filename() . substr($_FILES['attr_image_list']['name'][$key], strpos($_FILES['attr_image_list']['name'][$key], '.'));
$orgImg=DATA_DIR . "/color_pro/".$filename;

$thumb_url = $image->make_thumb($tempname,880,566,$path.DATA_DIR . "/color_pro/","#f6f6f6"); 
$thumbs_url = $image->make_thumb($tempname,128,60,$path.DATA_DIR . "/color_pro/","#f6f6f6"); 
      ...
      if (!is_uploaded_file($tempname))
        {
                echo "<script type='text/javascript' language='JavaScript'>alert('图片不存在!')</script>";
                exit;
        }

      

             if(!in_array($_FILES["attr_image_list"]["type"][$key], $uptypes))
            {
               echo "<script type='text/javascript' language='JavaScript'>alert('文件类型不符!".$file['type']."')</script>";
               exit;
            }
           ...
           if(!move_uploaded_file ($tempname,$path.$orgImg))
           {
             echo "<script type='text/javascript' language='JavaScript'>alert('移动文件出错')</script>";
             exit;
           }
           ...
           if(!$_FILES['attr_image_left_list']['error'] > 0){
          ...
          if($_FILES['attr_image_left_list']['name'][$key]==''){

          if($_POST['imgleft'][$key]!=''){

            $attr_leftimg=$_POST['imgleft'][$key];
			$thumb_lefturl=$_POST['imgleftthmp'][$key];
			$thumbs_lefturl=$_POST['attr_left_thumps'][$key];
        }else{
        ...
        //  $aa=time().$_FILES['attr_image_list']['name'][$key];
          ...
          $tempname=$_FILES["attr_image_left_list"]["tmp_name"][$key];
$path= ROOT_PATH;
$image = new cls_image();
$filename =$image->random_filename() . substr($_FILES['attr_image_left_list']['name'][$key], strpos($_FILES['attr_image_left_list']['name'][$key], '.'));
$orgleftImg=DATA_DIR . "/color_pro/".$filename;

$thumb_lefturl = $image->make_thumb($tempname,880,566,$path.DATA_DIR . "/color_pro/","#f6f6f6"); 
$thumbs_lefturl = $image->make_thumb($tempname,128,60,$path.DATA_DIR . "/color_pro/","#f6f6f6"); 
      ...
      if (!is_uploaded_file($tempname))
        {
             ...
             if(!in_array($_FILES["attr_image_left_list"]["type"][$key], $uptypes))
            {
               echo "<script type='text/javascript' language='JavaScript'>alert('文件类型不符!".$file['type']."')</script>";
               exit;
            }
           ...
           if(!move_uploaded_file ($tempname,$path.$orgleftImg))
           {
             echo "<script type='text/javascript' language='JavaScript'>alert('移动文件出错')</script>";
             exit;
           }
		   
		     $attr_leftimg=$orgleftImg;
     }

          }
               
               
			    $attr_envalue=$_POST['attr_envalue_list'][$key];
                $attr_value = $_POST['attr_value_list'][$key];
                $attr_price = $_POST['attr_price_list'][$key];
				
	         $attr_usa_price=$_POST['attr_usa_price_list'][$key];
                ...
                if (!empty($attr_value))
                {
                    ...
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
                        ...
                        $goods_attr_list[$attr_id][$attr_value]['attr_price'] = $attr_price;
						            ...
						            $goods_attr_list[$attr_id][$attr_value]['attr_usa_price'] = $attr_usa_price;
						
						$goods_attr_list[$attr_id][$attr_value]['attr_org'] = $attr_img;
					    $goods_attr_list[$attr_id][$attr_value]['attr_thump'] =$thumb_url;
						    $goods_attr_list[$attr_id][$attr_value]['attr_thumps'] =$thumbs_url;
						    $goods_attr_list[$attr_id][$attr_value]['color'] =$color;
							   $goods_attr_list[$attr_id][$attr_value]['attr_envalue'] =$attr_envalue;
							      $goods_attr_list[$attr_id][$attr_value]['frame_des'] =$frame_des;
								  $goods_attr_list[$attr_id][$attr_value]['xinghao'] =$xinghao;
							     $goods_attr_list[$attr_id][$attr_value]['glass_des'] =$glass_des;
								 
								     $goods_attr_list[$attr_id][$attr_value]['frame_endes'] =$frame_endes;
							     $goods_attr_list[$attr_id][$attr_value]['glass_endes'] =$glass_endes;
							   		$goods_attr_list[$attr_id][$attr_value]['attr_left_org'] = $attr_leftimg;
					    $goods_attr_list[$attr_id][$attr_value]['attr_left_thump'] =$thumb_lefturl;
						    $goods_attr_list[$attr_id][$attr_value]['attr_left_thumps'] =$thumbs_lefturl;
        ...
        {  
            ...
            { 
                if ($info['sign'] == 'insert')
                {
				 ...
				 //  $imageinfos=handle_isert_image($info['attr_image_name'],$info['attr_image_tmpname'],100,100);
				
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value,attr_envalue,cat_id ,attr_price,attr_thumb,attr_thumps,attr_org,color,attr_left_thumb,attr_left_thumps,attr_left_org,frame_des,xinghao,glass_des,frame_endes,glass_endes,attr_usa_price)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value','$info[attr_envalue]','$catgory_id', '$info[attr_price]','$info[attr_thump]','$info[attr_thumps]','$info[attr_org]','$info[color]','$info[attr_left_thump]','$info[attr_left_thumps]','$info[attr_left_org]','$info[frame_des]','$info[xinghao]','$info[glass_des]','$info[frame_endes]','$info[glass_endes]','$info[attr_usa_price]')";
                }
                elseif ($info['sign'] == 'update')
                {
				
				  $sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]',attr_envalue='$info[attr_envalue]',attr_thumb='$info[attr_thump]',attr_thumps='$info[attr_thumps]',attr_org='$info[attr_org]',cat_id='$catgory_id',color='$info[color]',attr_left_thumb='$info[attr_left_thump]',attr_left_thumps='$info[attr_left_thumps]',attr_left_org='$info[attr_left_org]',frame_des='$info[frame_des]',xinghao='$info[xinghao]',frame_endes='$info[frame_endes]',glass_des='$info[glass_des]',glass_endes='$info[glass_endes]',attr_usa_price='$info[attr_usa_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
    ...
    /* 处理相册图片 */
    handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file'],880,566);
...
//-- 修改产品封面图
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_goods_thumb')
{
    check_authz_json('goods_manage');

    $goods_id   = intval($_POST['id']);
    $goods_thumb = json_str_iconv(trim($_POST['val']));

    if ($exc->edit("goods_thumb = '$goods_thumb', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result(stripslashes($goods_thumb));
    }
}
/*------------------------------------------------------ */
//-- 修改商品价格
...
}elseif ($_REQUEST['act'] == 'toggle_discount')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_discount         = intval($_POST['val']);

    if ($exc->edit("is_discount = '$is_discount', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_discount);
    }
}
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        if (!empty($row['img_original']))
        {
		  ...
		  if (!empty($row['thumb_url_t']))
        {
            @unlink('../' . $row['thumb_url_t']);
        }
    }
...
}else if($_REQUEST['act'] == 'drop_color'){

  check_authz_json('goods_manage');

    $color_id = empty($_REQUEST['color_id']) ? 0 : intval($_REQUEST['color_id']);

	  $sql = "DELETE FROM " . $GLOBALS['ecs']->table('goods_color') . " WHERE id = '$color_id' LIMIT 1";
    $GLOBALS['db']->query($sql);

    clear_cache_files();
    make_json_result($color_id);
}
...
elseif ($_REQUEST['act'] == 'edit_product_sku')
{
    check_authz_json('goods_manage');

    $product_id       = intval($_POST['id']);
    $product_sku       = json_str_iconv(trim($_POST['val']));
    $product_sku       = ($_LANG['n_a'] == $product_sku) ? '' : $product_sku;
	if(!empty($product_sku)){
	
	 $sku=product_stock($product_sku);
	
	}
   
    if (check_product_sku_exist($product_sku, $product_id))
    {
      make_json_error("添加失败,sku码已存在!");
    }
    ...
    /* 修改 */
    $sql = "UPDATE " . $ecs->table('products') . " SET product_sku = '$product_sku',product_number='$sku' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        if (update_goods_stock($product['goods_id'], $sku - $product['product_number']))
        {
            clear_cache_files();
            make_json_result($product_sku);
        }
    }
}
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    foreach($product['product_sn'] as $key => $value)
    {
		...
		$sku=product_stock($product['product_sku'][$key]);
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('products') . " (goods_id, goods_attr, product_sn, product_number,product_sku)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $sku . "','" . $product['product_sku'][$key] . "')";
...
//$_FILES['edit_file']['name'][$t],$_FILES['edit_file']['tmp_name'][$t]

function handle_isert_image($image_name,$tempname,$width,$height)
{ 
...
$orgImg=DATA_DIR . "/color_pro/".$filename;
$thumb_url = $image->make_thumb($tempname,$articel_width,$articel_height,$path.DATA_DIR . "/color_pro/","#f6f6f6"); 
...
$imginfo['orgImg']=$orgImg;

$imginfo['thumb_url']=$thumb_url;
