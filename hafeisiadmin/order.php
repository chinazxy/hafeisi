...
require_once(ROOT_PATH . 'includes/lib_return.php');
/*------------------------------------------------------ */
//-- 订单查询
/*------------------------------------------------------ */
...
}elseif('confirm_refund' == $_REQUEST['act'])
{
	...
	$links[] = array('text' => '退货申请列表', 'href' => 'order.php?act=refund_list');
    sys_msg("成功处理该退货申请", 1, $links);
}
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if (isset($_REQUEST['order_id']))
    {
		...
		$order['inv_address']=getRegionName($order['country']).'&nbsp;&nbsp;'.getRegionName($order['inv_province']).'&nbsp;&nbsp;'.getRegionName($order['inv_city']).'&nbsp;&nbsp;'.getRegionName($order['inv_district']).'&nbsp;&nbsp;'.$order['inv_address'].$order['inv_room'];
		if($order['inv_tel']!=''){
			$order['phone']=$order['inv_tel'];
		}else{
			$order['phone']=$order['inv_number'];
		}
	...
	//$goods_qindan=array();//售货清单
	foreach($goods_list as $k=>$v){
		$goods_list[$k]['qindan']=unserialize($v['dingzhi_number']);
		$goods_list[$k]['dingzhi_attr_list_str']=unserialize($v['dingzhi_attr_list']);
	}
	if($order['pay_status']==2&&$order['extension_code']=="dingzhi"){
		
		$smarty->assign('qindanshow',true);
	}
    ...
    while ($row = $db->fetchRow($res))
    {
	 ...
	 /*$sql = "SELECT * FROM " . $ecs->table('order_action') . " WHERE order_id = '$order[order_id]' order by rec_id desc";
    $order_goods_list = $db->getAll($sql);
	var_dump($order_goods_list);
	exit;*/
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    /* 取得参数 step */
    $step_list = array('user', 'edit_goods', 'add_goods', 'goods', 'consignee', 'shipping', 'payment', 'other', 'money', 'invoice','fapiao','pay_dingzhigoods');
    ...
    elseif ('consignee' == $step)
    {
        ...
        elseif (isset($_POST['finish']))
        {
            ...
            if ('edit' == $step_act && exist_real_goods($order_id))
            {
                ...
                $shipping_list = available_shipping_list_t($region_id_list);
	...
	$inv_email=isset($_POST['inv_email'])?trim($_POST['inv_email']):"";
	$inv_surname=isset($_POST['inv_surname'])?trim($_POST['inv_surname']):"";
    $inv_name=isset($_POST['inv_name'])?trim($_POST['inv_name']):'';
	$inv_types=isset($_POST['inv_types'])?trim($_POST['inv_types']):0;
	$inv_tel='';
	$inv_number='';
	...
	$inv_address=isset($_POST['inv_address'])?trim($_POST['inv_address']):'';
	
	$inv_room=isset($_POST['inv_room'])?trim($_POST['inv_room']):'';
	
	$inv_company_name=isset($_POST['inv_company_name'])?trim($_POST['inv_company_name']):'';
	
	$inv_quhao=isset($_POST['inv_quhao'])?trim($_POST['inv_quhao']):'';
	
	
	$inv_zipcode=isset($_POST['inv_zipcode'])?trim($_POST['inv_zipcode']):'';
	
	$inv_province=isset($_POST['inv_province'])?trim($_POST['inv_province']):'';
	
	$inv_city=isset($_POST['inv_city'])?trim($_POST['inv_city']):'';
	
	$inv_district=isset($_POST['inv_district'])?trim($_POST['inv_district']):'';
	
	$id=intval($_GET['order_id']);
	//$fp_time=isset($_POST['start_time'])?strtotime($_POST['start_time']):'';
    ...
    elseif ('pay_dingzhigoods' == $step)
    {
		$sql='';
		$biaodan=$_POST['value'];
		foreach($biaodan as $k=>$v){
			foreach($v as $key=>$val){
				$biaodan[$k][$key]=trim($val);
			}			
		}
		
		foreach($_POST['rec_id'] as $k=>$v){
			     
		     $arr_pei=array();
		     $peijian=substr(trim($_POST['peijian'][$k]),0,strlen(trim($_POST['peijian'][$k]))-1);
			 if($peijian != ''){
			   $arr_pei=explode(',',$peijian);
			 }
			 $arrpp=array();
			 foreach($arr_pei as $ka=>$va){
				$arrpp[$ka]['number']=trim($va); 
			
			 }
			 $biaodan[$k]['peijian']=$arrpp;
			 $sql = "UPDATE " .$GLOBALS['ecs']->table('order_goods'). " SET ".
              "dingzhi_number  = '".serialize($biaodan[$k])."' ".
            "WHERE rec_id = '".intval($v)."'";		
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    /* 取得参数 step */
    $step_list = array('user', 'goods', 'consignee', 'shipping', 'payment', 'other', 'money','fapiao','pay_dingzhigoods');
    ...
    elseif ('goods' == $step)
    {
		...
		foreach($goods_list as $key=>$val){
			$goods_list[$key]['dingzhi_attr_list_str']=unserialize($val['dingzhi_attr_list']);
		}
...
//        }
    ...
    }elseif('pay_dingzhigoods' == $step)
    {
	   ...
	   $goods_list = order_goods($order_id);
		...
		$dinzhi_num_arr=array('zjk_number','jk','jp','jt','jtt','sj','qk','jk','jp_r','jt_r','jtt_r','sj','qk_r');
		foreach($goods_list as $key=>$val){
			//$goods_list[$key]['dingzhi_attr_list_str']=unserialize($val['dingzhi_attr_list']);
			$goods_list[$key]['qindan']=unserialize($val['dingzhi_number']);
		    $goods_list[$key]['dingzhi_attr_list_str']=unserialize($val['dingzhi_attr_list']);
			for($i=0;$i<count($dinzhi_num_arr);$i++){
			  if(!isset($goods_list[$key]['qindan'][$dinzhi_num_arr[$i]])){
				  $goods_list[$key]['qindan'][$dinzhi_num_arr[$i]]=false;
			  }
			}
		}
        ...
        $smarty->assign('step','pay_dingzhigoods');
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    $goods_list = array();
    $goods_attr = array();
	 //g.dingzhi_attr, g.dingzhi_img, g.dingzhi_thumb, g.dingzhi_attr_list, g.extension_code,
    $sql = "SELECT o.*, g.goods_thumb, g.goods_number AS storage, o.goods_attr,i.order_d_sn,IFNULL(b.brand_name, '') AS brand_name " .
            "FROM " . $ecs->table('order_goods') . " AS o ".
            "LEFT JOIN " . $ecs->table('goods') . " AS g ON o.goods_id = g.goods_id " .
            "LEFT JOIN " . $ecs->table('brand') . " AS b ON g.brand_id = b.brand_id " .
			"INNER JOIN " . $ecs->table('order_info') . " AS i ON o.order_id = i.order_id ".
...
function order_list($refund_ex="")
{
    ...
    if ($result === false)
    {
	  ...
	  $sql = "SELECT action_list FROM " . $GLOBALS['ecs']->table('admin_user') . " WHERE user_id = '$_SESSION[admin_id]'";
		    $action_list= $GLOBALS['db']->getOne($sql);
			$arr=explode(',',$action_list);//分解字符串为数组
			//$agency='';
			//var_dump($arr);
		  // var_dump(in_array('putongguanli',$arr));
		   //var_dump(in_array('dingzhiguanli',$arr));
		//检查权限
		//var_dump($where);
		//exit;
		if(in_array('dingzhiguanli',$arr) && in_array('putongguanli',$arr)){//普通订单和定制订单都管理 直接跳出
		   	$agency='quan';//备注agency
		}
	    elseif (in_array('dingzhiguanli',$arr))//定制订单管理
        {
            $where .= " AND o.extension_code = 'dingzhi' ";
        }elseif(in_array('putongguanli',$arr)){//普通订单管理
			$where .= " AND o.extension_code = '' ";
		}
        if ($agency_id > 0)
        {
    ...
    foreach ($row AS $key => $value)
    {
		...
		//$row[$key]['dingzhi']=$GLOBALS['db']->getAll($sql);
