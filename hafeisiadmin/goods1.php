...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    /* 创建 html editor */
	 ...
	 create_html_editor2('goods_size', $goods['goods_size']);
     create_html_editor4('goods_xijie', $goods['goods_xijie']);
      create_html_editor8('goods_brief', $goods['goods_brief']);
     create_html_editor5('goods_material',$goods['goods_material']);
	 
	  create_html_editor7('otherinfo',$goods['otherinfo']);
    /* 模板赋值 */
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        ...
        if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']) || $is_url_goods_img)
        {
            ...
            if ($_CFG['auto_generate_gallery'])
            {
                ...
                {
                    $gallery_thumb = $image->make_thumb('../' . $img, 639,639);
    ...
    else
    {
        ...
        if ($proc_thumb && isset($_POST['auto_thumb']) && !empty($original_img))
        {
            ...
            if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0)
            {
                $goods_thumb = $image->make_thumb('../' . $original_img, 355, 355);
				$goods_photo_thumb= $image->make_thumb('../' . $original_img, 639, 639);
                if ($goods_thumb === false)
                {
                    sys_msg($image->error_msg(), 1, array(), false);
                }
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,xingji, suppliers_id, goods_material,goods_xijie,otherinfo,tmail_url,goods_sku)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$_POST[xingji]', '$suppliers_id','$_POST[goods_material]','$_POST[goods_xijie]','$_POST[otherinfo]','$_POST[tmail_url]','$goods_sku')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,goods_material,goods_xijie,otherinfo,tmail_url,goods_sku)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img','$goods_photo_thumb', ".
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$_POST[goods_material]','$_POST[goods_xijie]','$_POST[otherinfo]','$_POST[tmail_url]',$goods_sku)";
        }
    }
    else
    {
...
$thumb_url = $image->make_thumb($tempname,639,639,$path.DATA_DIR . "/color_pro/"); 
$thumbs_url = $image->make_thumb($tempname,34,34,$path.DATA_DIR . "/color_pro/"); 
            ...
            { 
                if ($info['sign'] == 'insert')
                {
				 ...
				 //  $imageinfos=handle_isert_image($info['attr_image_name'],$info['attr_image_tmpname'],100,100);
                    ...
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value,cat_id ,attr_price,attr_thumb,attr_thumps,attr_org,color)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value','$catgory_id', '$info[attr_price]','$info[attr_thump]','$info[attr_thumps]','$info[attr_org]','$info[color]')";
                }
                elseif ($info['sign'] == 'update')
                {
				
				  $sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]',attr_thumb='$info[attr_thump]',attr_thumps='$info[attr_thumps]',attr_org='$info[attr_org]',cat_id='$catgory_id',color='$info[color]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
    ...
    /* 处理相册图片 */
    handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file'],639,639);
...
$orgImg=DATA_DIR . "/color_pro/".$filename;
$thumb_url = $image->make_thumb($tempname,$articel_width,$articel_height,$path.DATA_DIR . "/color_pro/"); 
