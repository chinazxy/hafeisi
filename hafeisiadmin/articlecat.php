...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('form_action', 'insert');
    	 create_html_editor2('goods_size',"");
    assign_query_info();
...
//
    ...
    if ($_POST['parent_id'] > 0)
    {
     ...
     $adimage=handle_ad_image($_FILES['adimae']);
	 $adimages=handle_ad_image($_FILES['adimaes']);
    $sql = "INSERT INTO ".$ecs->table('article_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav,cat_enname,label,bg,bgs)
           VALUES ('$_POST[cat_name]', '$cat_type',  '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]','$_POST[cat_enname]','$_POST[goods_size]','$adimage','$adimages')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT cat_id, cat_name,cat_enname,label,bg,bgs, cat_type, cat_desc, show_in_nav, keywords, parent_id,sort_order FROM ".
    ...
    unset($options);
	 create_html_editor2('goods_size',$cat['label']);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($cat_type == 1 || $cat_type == 5)
    {
        ...
        else
        {
		  ...
		  if (empty($_FILES['adimae']['error']) || (!isset($_FILES['adimae']['error']) && isset($_FILES['adimae']['tmp_name']) && $_FILES['adimae']['tmp_name'] != 'none'))
     {
    	$adimage=handle_ad_image($_FILES['adimae']);
		
		}else{
		$adimage=$_POST['hide_adimg'];
		
		}
		
	  if (empty($_FILES['adimaes']['error']) || (!isset($_FILES['adimaes']['error']) && isset($_FILES['adimaes']['tmp_names']) && $_FILES['adimaes']['tmp_names'] != 'none'))
     {
    	$adimages=handle_ad_image($_FILES['adimaes']);
		
		}else{
		$adimages=$_POST['hide_adimgs'];
		
		}	
		
    $dat = $db->getOne("SELECT cat_name, show_in_nav FROM ". $ecs->table('article_cat') . " WHERE cat_id = '" . $_POST['id'] . "'");
    if ($exc->edit("cat_name = '$_POST[cat_name]',cat_enname='$_POST[cat_enname]',label='$_POST[goods_size]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]',bg='$adimage',bgs='$adimages'",  $_POST['id']))
    {
...
elseif ($_REQUEST['act'] == 'remove')
{
 ...
 $sql="SELECT * FROM ".$ecs->table('article_cat')." where cat_id=".$id;

 $image=$db->getRow($sql); 
 
 
    if ($image['bg'] != '' && is_file('../' . $image['bg']))
    {
        @unlink('../' . $image['bg']);
    }
	    if ($image['bgs'] != '' && is_file('../' . $image['bgs']))
    {
        @unlink('../' . $image['bgs']);
    }
	
$sql="Update  ".$ecs->table('article_cat')." SET bg='' where cat_id=".$id;
...
if($state){
 ...
 $links[0]['text']    ="返回";
  $links[0]['href']    = 'articlecat.php?act=edit&id='.$id;
...
}else if($_REQUEST['act']=='deleteimgs'){
...
$sql="Update  ".$ecs->table('article_cat')." SET bgs='' where cat_id=".$id;
...
if($image_files['error']==0){
$filename =$images->random_filename() . substr($image_files['name'], strpos($image_files['name'], '.'));

$orgImg=DATA_DIR . "/ad/".$filename;
//原图保存
if (!move_upload_file($image_files['tmp_names'],$path.$orgImgs))
    {
        return false;
    }
}
