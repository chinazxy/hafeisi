...
<div class="main-div">
<form method="post" action="articlecat.php" name="theForm"  onsubmit="return validate()" enctype="multipart/form-data">
<table cellspacing="1" cellpadding="3" width="100%">
  <tr>
    ...
    <td><input type="text" name="cat_enname" maxlength="60" size = "30" value="<?php echo htmlspecialchars($this->_var['cat']['cat_enname']); ?>" /><?php echo $this->_var['lang']['require_field']; ?></td>
  </tr>
  ...
  <tr>
	...
	<?php if ($this->_var['cat']['cat_id'] == 2): ?>
       <tr>
        <td class="narrow-label">分类背景图片</td>
        <td><input type="file" name="adimae" /><input type="hidden" value="<?php echo $this->_var['cat']['bg']; ?>" name="hide_adimg" /><?php if ($this->_var['cat']['bg'] != ''): ?><div id="adimg"> <img src="../<?php echo $this->_var['cat']['bg']; ?>" width="100px" height="100px" /><a  onclick="deletImg('<?php echo $this->_var['cat']['cat_id']; ?>')" href='JavaScript:void(0);'>删除</a></div> <?php endif; ?>
		<span style="color:#FF0000">分类背景图片1920*高300</span>
		</td>
		
      </tr>
	  <?php endif; ?>
	  
	  	<?php if ($this->_var['cat']['cat_id'] == 11): ?>
       <tr>
        <td class="narrow-label">新闻背景图片</td>
        <td><input type="file" name="adimae" /><input type="hidden" value="<?php echo $this->_var['cat']['bg']; ?>" name="hide_adimg" /><?php if ($this->_var['cat']['bg'] != ''): ?><div id="adimg"> <img src="../<?php echo $this->_var['cat']['bg']; ?>" width="100px" height="100px" /><a  onclick="deletImg('<?php echo $this->_var['cat']['cat_id']; ?>')" href='JavaScript:void(0);'>删除</a></div> <?php endif; ?>
		<span style="color:#FF0000">新闻背景图片1180*高200</span>
		</td>
		
      </tr>
	  <?php endif; ?>
	  <?php if ($this->_var['cat']['cat_id'] != 11 && $this->_var['cat']['cat_id'] != 2): ?>
       <tr>
        <td class="narrow-label">新闻背景图片</td>
        <td><input type="file" name="adimae" /><input type="hidden" value="<?php echo $this->_var['cat']['bg']; ?>" name="hide_adimg" /><?php if ($this->_var['cat']['bg'] != ''): ?><div id="adimg"> <img src="../<?php echo $this->_var['cat']['bg']; ?>" width="100px" height="100px" /><a  onclick="deletImg('<?php echo $this->_var['cat']['cat_id']; ?>')" href='JavaScript:void(0);'>删除</a></div> <?php endif; ?>
	  ...
	  <?php endif; ?>
		<?php if ($this->_var['cat']['cat_id'] == 2): ?>
		       <tr>
        <td class="narrow-label">分类背景手机图片</td>
        <td><input type="file" name="adimaes" /><input type="hidden" value="<?php echo $this->_var['cat']['bgs']; ?>" name="hide_adimgs" /><?php if ($this->_var['cat']['bg'] != ''): ?><div id="adimgs"> <img src="../<?php echo $this->_var['cat']['bgs']; ?>" width="100px" height="100px" /><a  onclick="deletImgs('<?php echo $this->_var['cat']['cat_id']; ?>')" href='JavaScript:void(0);'>删除</a></div> <?php endif; ?>
		<span style="color:#FF0000">分类背景手机图片960*高330</span>
		</td>
...
function deletImg(id){
  ...
  if(confirm("您确定要删除该图片吗?")){
    ...
    location.href="articlecat.php?act=deleteimg&id="+id;
...
function deletImgs(id){

  if(confirm("您确定要删除该图片吗?")){
  
    location.href="articlecat.php?act=deleteimgs&id="+id;
