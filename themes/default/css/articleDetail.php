...
if (!$smarty->is_cached('newsdetail.dwt', $cache_id))
{
    /* 文章详情 */
    ...
    if (!empty($article_info['link']) && $article_info['link'] != 'http://' && $article_info['link'] != 'https://')
    {
        ecs_header("location:$article_info[link]\n");
        exit;
    }
	 ...
	 $smarty->assign("imglist",getRelateImg($_REQUEST['id']));
  ...
  //  $smarty->assign('top_goods',        get_top10());    // 销售排行
   // $smarty->assign('best_goods',       get_recommend_goods('best'));       // 推荐商品
    ...
    $smarty->assign('keywords',     htmlspecialchars($article_info['keywords']));
    $smarty->assign('description', htmlspecialchars($article_info['description']));

    $catlist = array();
    foreach(get_article_parent_cats($article_info['cat_id']) as $k=>$v)
    {
        $catlist[] = $v['cat_id'];
    }
    ...
    $position = assign_ur_here($article_info['cat_id'], $article_info['title']);
    ...
    /* 上一篇下一篇文章 */
    $next_article = $db->getRow("SELECT article_id, title FROM " .$ecs->table('article'). " WHERE article_id > $article_id AND cat_id=$article_info[cat_id] AND is_open=1 LIMIT 1");
    ...
    $prev_aid = $db->getOne("SELECT max(article_id) FROM " . $ecs->table('article') . " WHERE article_id < $article_id AND cat_id=$article_info[cat_id] AND is_open=1");
    if (!empty($prev_aid))
    {
    ...
    $smarty->display('newsdetail.dwt', $cache_id);
...
function getRelateImg($id){


 $sql = 'SELECT * ' .'FROM ' . $GLOBALS['ecs']->table('news_img') ." where cat_id=".$id;
 
 $imgarray=$GLOBALS['db']->getAll($sql);
