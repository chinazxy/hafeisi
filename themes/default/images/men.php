...
$size = isset($_CFG['page_size'])  && intval($_CFG['page_size']) > 0 ? intval($_CFG['page_size']) : 9;
...
/* 页面的缓存ID */
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        $price_grade[0]['url'] = build_uri('men', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
...
$smarty->assign("banner",$banner); 
    assign_pager('product',  $cat_id, $count, $size, $sort, $order, $page, '', $price_min, $price_max, $display, $filter_attr_str); // 分页
    assign_dynamic('category'); // 动态内容

$smarty->display('productwomen.dwt', $cache_id);
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id,cat_plot,cat_enname FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function getchildCategoty($cat_id,$filter_attr_str){

  $sql="SELECT * FROM ".$GLOBALS['ecs']->table('category')." where parent_id=".$cat_id." order by sort_order asc";
